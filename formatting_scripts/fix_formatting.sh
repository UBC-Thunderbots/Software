...
# This file is autogenerated by CubeMX and doesn't play well with clang-format 8 or 10
# We don't need to edit this file, so it can be safely ignored
EXCLUDED_FILES=("firmware_new/boards/frankie_v1/stm32h7xx_hal_conf.h")
...
function run_clang_format () {
    ...
    # Generate args to exlcude files
    FILE_EXCLUSION_STRING=""
    for value in "${EXCLUDED_FILES[@]}"
    do
        FILE_EXCLUSION_STRING="$FILE_EXCLUSION_STRING -path $CURR_DIR/../src/$value -prune -o"
    done

    FIND_ARGS="$FILE_EXCLUSION_STRING $EXTENSION_STRING"
    
    echo "find $CURR_DIR/../src/ ${FIND_ARGS::-2}"
