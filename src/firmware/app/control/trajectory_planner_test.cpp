...
TEST_F(TrajectoryPlannerTest, test_constant_time_interpolation)
{
    ...
    // Test by checking the length between points
    // and the length of the total path
    Trajectory const_interp_trajectory = interpolate_constant_time_trajectory_segmentation(trajectory, num_segments, 0.001);
    
    EXPECT_NEAR(trajectory[num_segments-1].position.x, const_interp_trajectory.trajectory_elements[const_interp_trajectory.num_elements-1].position.x, 0.001);
    EXPECT_NEAR(trajectory[num_segments-1].position.y, const_interp_trajectory.trajectory_elements[const_interp_trajectory.num_elements-1].position.y, 0.001);
    EXPECT_NEAR(trajectory[num_segments-1].time, const_interp_trajectory.trajectory_elements[const_interp_trajectory.num_elements-1].time, 0.001);

    EXPECT_NEAR(trajectory[0].position.x, const_interp_trajectory.trajectory_elements[0].position.x, 0.001 );
    EXPECT_NEAR(trajectory[0].position.y, const_interp_trajectory.trajectory_elements[0].position.x, 0.001 );
    EXPECT_NEAR(trajectory[0].time, const_interp_trajectory.trajectory_elements[0].time, 0.001);
}