...
class TrajectoryPlannerTest : public testing::Test
{
   protected:
    ...
    static std::vector<double> getVelocityFromTrajectory(TrajectoryElement_t trajectory[],
                                                         int num_elements,
                                                         double inital_speed)
    {
    ...
    static std::vector<double> getAccelerationFromVelocity(
        std::vector<double> velocity, TrajectoryElement_t trajectory[], int num_elements)
    {
...
TEST_F(TrajectoryPlannerTest, path_length_and_start_point_end_point_test)
{
    ...
    {
        double length =
            sqrt(pow(trajectory[i].position.x - trajectory[i + 1].position.x, 2) +
                 pow(trajectory[i].position.y - trajectory[i + 1].position.y, 2));
    ...
    EXPECT_NEAR(trajectory[num_segments - 1].position.x,
                shared_polynomial2d_getValueOrder3(path, t_end).x, 0.01);
    EXPECT_NEAR(trajectory[num_segments - 1].position.y,
                shared_polynomial2d_getValueOrder3(path, t_end).y, 0.01);

    EXPECT_NEAR(trajectory[0].position.x,
                shared_polynomial2d_getValueOrder3(path, t_start).x, 0.01);
    EXPECT_NEAR(trajectory[0].position.x,
                shared_polynomial2d_getValueOrder3(path, t_start).y, 0.01);
}
...
TEST_F(TrajectoryPlannerTest, dynamics_dont_exceed_maximums_straight_line)
{
    ...
    std::vector<double> velocity =
        getVelocityFromTrajectory(trajectory, num_segments, initial_speed);
    std::vector<double> acceleration =
        getAccelerationFromVelocity(velocity, trajectory, num_segments);
       ...
       path_length_and_start_point_end_point_test_reverse_direction)
{
    ...
    {
        double length =
            sqrt(pow(trajectory[i + 1].position.x - trajectory[i].position.x, 2) +
                 pow(trajectory[i + 1].position.y - trajectory[i].position.y, 2));
        segment_length_sum += length;
    }
    EXPECT_NEAR(segment_length_sum, arc_segment_length, 0.01);
    EXPECT_NEAR(trajectory[num_segments - 1].position.x,
...
TEST_F(TrajectoryPlannerTest, dynamics_dont_exceed_maximums_curved_path)
{
    ...
    EXPECT_NEAR(trajectory[num_segments - 1].position.x,
                shared_polynomial2d_getValueOrder3(path, t_end).x, 0.025);
    EXPECT_NEAR(trajectory[num_segments - 1].position.y,
                shared_polynomial2d_getValueOrder3(path, t_end).y, 0.025);

    EXPECT_NEAR(trajectory[0].position.x,
                shared_polynomial2d_getValueOrder3(path, t_start).x, 0.025);
    EXPECT_NEAR(trajectory[0].position.x,
