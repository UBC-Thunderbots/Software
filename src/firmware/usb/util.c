...
const usb_interface_descriptor_t *uutil_find_interface_descriptor(const usb_configuration_descriptor_t *config, unsigned int interface, unsigned int altsetting) {
	...
	while (cur - base < config->wTotalLength) {
		if (cur[1U] == USB_DTYPE_INTERFACE) {
			const usb_interface_descriptor_t *idesc = (const usb_interface_descriptor_t *) cur;
			if (idesc->bInterfaceNumber == interface && idesc->bAlternateSetting == altsetting) {
				return idesc;
			}
		}
		cur += cur[0U];
...
const udev_endpoint_info_t *uutil_find_endpoint_info(unsigned int ep) {
	...
	const udev_interface_info_t *iinfo = interface < uep0_current_configuration->descriptors->bNumInterfaces ? uep0_current_configuration->interfaces[interface] : 0;
	if (iinfo) {
