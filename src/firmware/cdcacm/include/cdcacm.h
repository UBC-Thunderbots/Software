 ...
 */
typedef enum {
	CDCACM_CLASS_CONTROL = 0x02U, ///< The class for a control interface or interface association
 ...
 */
typedef enum {
	CDCACM_SUBCLASS_CONTROL = 0x02U, ///< The subclass for a control interface or interface association
 ...
 */
typedef enum {
	CDCACM_PROTOCOL_CONTROL = 0x01U, ///< The protocol for a control interface or interface association
...
typedef struct __attribute__((packed)) {
	uint8_t bLength; ///< The length of this descriptor (\c sizeof(cdcacm_interface_association_descriptor_t))
	uint8_t bDescriptorType; ///< The type of this descriptor (\ref cdcacm_dtype_t "CDCACM_DTYPE_CS_INTERFACE")
	uint8_t bDescriptorSubtype; ///< The subtype of this descriptor (\ref cdcacm_dstype_t "CDCACM_DSTYPE_HEADER")
	uint16_t bcdCDC; ///< The version number of the CDC specification to which the device complies
...
typedef struct __attribute__((packed)) {
	uint8_t bLength; ///< The length of this descriptor (\c sizeof(cdcacm_interface_association_descriptor_t))
	uint8_t bDescriptorType; ///< The type of this descriptor (\ref cdcacm_dtype_t "CDCACM_DTYPE_CS_INTERFACE")
	uint8_t bDescriptorSubtype; ///< The subtype of this descriptor (\ref cdcacm_dstype_t "CDCACM_DSTYPE_UNION")
...
typedef struct __attribute__((packed)) {
	uint8_t bLength; ///< The length of this descriptor (\c sizeof(cdcacm_interface_association_descriptor_t))
	uint8_t bDescriptorType; ///< The type of this descriptor (\ref cdcacm_dtype_t "CDCACM_DTYPE_CS_INTERFACE")
	uint8_t bDescriptorSubtype; ///< The subtype of this descriptor (\ref cdcacm_dstype_t "CDCACM_DSTYPE_CALL_MANAGEMENT")
	struct __attribute__((packed)) {
		unsigned call_management : 1; ///< Whether this device handles call management
		unsigned call_management_on_data : 1; ///< Whether call management data can use the data interface
		unsigned : 6;
	} bmCapabilities; ///< The device’s capabilities
	uint8_t bDataInterface; ///< The data interface used if call management is done on a data interface
...
typedef struct __attribute__((packed)) {
	uint8_t bLength; ///< The length of this descriptor (\c sizeof(cdcacm_interface_association_descriptor_t))
	uint8_t bDescriptorType; ///< The type of this descriptor (\ref cdcacm_dtype_t "CDCACM_DTYPE_CS_INTERFACE")
	uint8_t bDescriptorSubtype; ///< The subtype of this descriptor (\ref cdcacm_dstype_t "CDCACM_DSTYPE_ABSTRACT_CONTROL_MANAGEMENT")
	struct __attribute__((packed)) {
		unsigned comm_features : 1; ///< Whether the device supports communication feature requests
		...
		unsigned connection_notify : 1; ///< Whether the device supports connection notifications
		unsigned : 4;
	} bmCapabilities; ///< The device’s capabilities
 ...
 *
 * This structure begins with an interface association descriptor and includes all the interfaces and endpoints.
 * The device descriptor should therefore indicate that the device uses interface association descriptors.
 */
typedef struct __attribute__((packed)) {
	usb_interface_association_descriptor_t iad; ///< The interface association descriptor
	usb_interface_descriptor_t control_intf; ///< The interface descriptor for the control interface
	...
	cdcacm_call_management_descriptor_t call_management; ///< The call management descriptor
	cdcacm_abstract_control_management_descriptor_t abstract_control_management; ///< The abstract control management descriptor
	usb_endpoint_descriptor_t notification_ep; ///< The endpoint descriptor for the notification endpoint
	usb_interface_descriptor_t data_intf; ///< The interface descriptor for the data interface
 ...
 *
 * \param[in] intf_num the interface number of the first interface to use (a CDC ACM uses two interfaces)
 * \param[in] iad_string the string index to use in the interface association descriptor
 * \param[in] control_string the string index to use in the control interface descriptor
 * \param[in] data_string the string index to use in the data interface descriptor
 * \param[in] notification_ep_num the endpoint number of an IN endpoint for notification messages
 * \param[in] data_out_ep_num the endpoint number of an OUT endpoint for data
 * \param[in] data_in_ep_num the endpoint number of an IN endpoint for data
 */
#define CDCACM_DESCRIPTORS_INIT(intf_num, iad_string, control_string, data_string, notification_ep_num, data_out_ep_num, data_in_ep_num) \
{ \
	.iad = { \
