This tool sends protobuf over serial to the STM32 to do simple arithmatic.It serves
    as a placeholder to demonstrate protobuf communication,
    as well as facilitate fw development
            .

        Simply modify robot.proto in firmware_new /
        proto and run this tool to get access to the updated protobuf fields.

        We are using proto2(
            mostly to be consistent with the ssl proto version) For more info on protobuf,
    please go here : https :  // developers.google.com/protocol-buffers/docs/cpptutorial

                              NOTE : We include robot.pb
                                         .h which contains the required info to create
                                     msgs defined in robot.proto.We link the cc_library w
        / c++ headers to this tool.On the microcontroller side we also include robot.pb.h,
    but we link the cc_library w / c headers.

                                   NOTE : Do not use libusb,
    ...
    void send_proto_over_serial(boost::asio::serial_port& port,
            const google::protobuf::Message& proto_msg)
     {
         ...
         std::this_thread::sleep_for(
             std::chrono::milliseconds((int)1e6 / baud_rate.value()));
     }
     }
     **TODO MOVE THIS NOTE
         : When broadcasting protobuf on the same subnet using a UDP TransferMedium,
         every device on* that subnet will receive the msg, which may include robots,
         this computer, other computers,
         or *robots that are on the same network but not on the
                         feild.This class WILL recieve the msg it sends *
                     due to the nature of UDP broadcast and
                 does NOT filter it out(when injected with a UDP TransferMedium)
