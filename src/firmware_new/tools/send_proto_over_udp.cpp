...
/* 
 * Communicate with the robot over UDP. This class broadcasts any
 * arbitrary proto msg
 *
 */
class RobotCommunicator
{
	public:
		...
		void send_proto(const Message& proto);
		void receive_proto_async(Message& proto, const std::function<void(const Message& proto)> func);
...
RobotCommunicator::RobotCommunicator()
	: io_service(),
      socket(io_service, {udp::v4(), 42070})
{
	io_service.run();
}

void RobotCommunicator::send_proto(const Message& proto){


}

void RobotCommunicator::async_recieve()
{
	socket.async_receive_from(
			boost::asio::buffer(recv_buffer), receiver_endpoint,
			boost::bind(&RobotCommunicator::handle_receive, this,
				boost::asio::placeholders::error,
				boost::asio::placeholders::bytes_transferred));
}

void RobotCommunicator::handle_receive(const boost::system::error_code& error, size_t bytes_transferred)
{
...
int main()
{
	RobotCommunicator robot_handle;
	robot_handle.receive();
	/*
