...
/**
 * Takes a circular buffer and extracts the bytes between last_parsed_byte_pos and
 * last_updated_byte_pos, accounting for "wrapped around" data, and puts it in the
 * linear buffer.
 *
 * @param last_parsed_byte_pos The position of the byte that was last parsed
 * @param last_updated_byte_pos The position of the last byte that was updated in the
 * buffer
...
io_ublox_odinw262_communicator_getUbloxResponseStatusFromCircularBuffer(
    size_t last_updated_byte_pos, size_t buffer_length, uint8_t* circular_buffer);
