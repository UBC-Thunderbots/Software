...
GrsimCommandPrimitiveVisitor::GrsimCommandPrimitiveVisitor(const Robot &robot, const Ball &ball)
    : robot(robot)
    , ball(ball)
{
}
...
{
    // Calculate the theoretical catch point based on how the distance to the ball and how fast the robot is moving.
    double distanceToBall = (ball.position() - robot.position()).len();
    double interceptonMargin = CatchPrimitive::STANDARD_INTERCEPT_MARGIN * catch_primitive.getMargin();
    Point finalDest;
    if (robot.velocity().len() != 0)
    {
        finalDest = ball.estimatePositionAtFutureTime(std::chrono::milliseconds((int) fabs(distanceToBall / robot.velocity().len()) * 1000 ));
        finalDest = Point(finalDest.x(),
                          finalDest.y());
    }
    else
    {
        // If Robot is not moving, estimate position based on a standard velocity of 1
        finalDest = ball.estimatePositionAtFutureTime(std::chrono::milliseconds((int) fabs(distanceToBall / 1) * 1000 ));
        finalDest = Point(finalDest.x(),
                          finalDest.y());
    }
