...
BallFilter::BallFilter(unsigned int min_buffer_size, unsigned int max_buffer_size) : _min_buffer_size(min_buffer_size),
                                                                                     _max_buffer_size(max_buffer_size),
                                                                                     ball_detection_buffer(
                                                                                             max_buffer_size) {}
...
void BallFilter::addNewDetectionsToBuffer(
    ...
    for (const auto &detection : new_ball_detections) {
        ...
        if (!ball_detection_buffer.empty()) {
            auto detection_with_smallest_timestamp = *std::min_element(ball_detection_buffer.begin(),
                                                                       ball_detection_buffer.end());
...
BallVelocityEstimate BallFilter::estimateBallVelocity(boost::circular_buffer<SSLBallDetection> ball_detections,
    ...
    // Sort the detections in increasing order before processing. This places the oldest detections (smallest timestamp)
    // at the front of the buffer, and the most recent detections (with the largest timestamp) at the end of the buffer
    ...
    for (int i = 0; i < ball_detections.size() - 1; i++) {
        for (int j = i + 1; j < ball_detections.size(); j++) {
            ...
            Point current_position = ball_regression_line ? closestPointOnLine(current_detection.position,
                                                                               *ball_regression_line)
                                                          : current_detection.position;
            Point previous_position = ball_regression_line ? closestPointOnLine(previous_detection.position,
                                                                                *ball_regression_line)
    ...
    double average_velocity_magnitude = velocity_magnitude_sum / ball_velocities.size();
    ...
    BallVelocityEstimate velocity_data({average_velocity, average_velocity_magnitude, min_max_average});

    return velocity_data;
}

size_t BallFilter::getAdjustedBufferSize(boost::circular_buffer<SSLBallDetection> ball_detections) {
    ...
    // Use the average of the min and max velocity magnitudes in the buffer. We use this rather than the average so we
    // can quickly respond to drastic changes in the ball velocity, such as when the ball goes from being stationary
    // to moving quickly (like when it's kicked). If the buffer is large, then it will take more time for the mean
    // speed to increase enough to start shrinking the buffer. However, the average of the min and max values will
    // immediately increase if the ball starts moving, so the buffer can start shrinking more quickly and increase
    // the filter response time to these sorts of changes.
    double min_max_magnitude_average = estimateBallVelocity(ball_detections).min_max_magnitude_average;

    // Between the min and max velocity magnitudes, we linearly scale the size of the buffer
    ...
    int buffer_size = max_buffer_size - static_cast<unsigned int>(std::floor(linear_scaling_factor * buffer_size_diff));

    return static_cast<size_t>(buffer_size);
}

LinearRegressionResults BallFilter::getLinearRegressionLine(boost::circular_buffer<SSLBallDetection> ball_detections) {
    // Sort the detections in increasing order before processing. This places the oldest detections (smallest timestamp)
    // at the front of the buffer, and the most recent detections (with the largest timestamp) at the end of the buffer
    std::sort(ball_detections.begin(), ball_detections.end());
    ...
    double regression_error = (A * regression_vector - b).norm() / (b.norm() + 1.0e-9); // norm() is L2 norm
...
Ball BallFilter::getLinearRegressionPositionAndVelocity(boost::circular_buffer<SSLBallDetection> ball_detections) {
    ball_detections.resize(getAdjustedBufferSize(ball_detections));
    ...
    // Flip the detection coordinates and perform the regression again. This is to account for edge cases where the
    // first regression does not work, such as a line directly along the y-axis. If we try perform regression with
    // the y coordinate as the dependant variable (eg. y = mx + b), lines such as x=3 cannot be represented. However,
    // if the flip the variables so we solve x = my + b then we can represent these lines. So by doing both forms
    // of regression we cover all our bases.
    ...
    // Because we flipped the coordinates of the input, we have to slip the coordinates of the output to get back to
    // our expected coordinate space
    y_vs_x_regression.regression_line = Line(
            Point(y_vs_x_regression.regression_line.getFirst().y(), y_vs_x_regression.regression_line.getFirst().x()),
    ...
    Line regression_line =
            x_vs_y_regression.regression_error < y_vs_x_regression.regression_error ? x_vs_y_regression.regression_line
    ...
    auto ball_velocity_data = estimateBallVelocity(ball_detections, regression_line);
    auto velocity_direction_along_regression_line = ball_velocity_data.average_velocity.project(regression_line.getSecond() - regression_line.getFirst());
    Vector filtered_velocity = velocity_direction_along_regression_line.norm(ball_velocity_data.average_velocity_magnitude);
