...
TEST_F(STPTest, test_play_assignment_from_one_play_to_another_when_current_play_invariant_no_longer_holds) {
    // Only the StopTestPlay should be applicable
    world = ::Test::TestUtil::setBallPosition(world, Point(-1, 1), Timestamp::fromSeconds(0));
    stp.getIntentAssignment(world);
    EXPECT_EQ(*(stp.getCurrentPlayName()), "Stop Test Play");
    ...
    // be applicable
    world = ::Test::TestUtil::setBallPosition(world, world.field().enemyCornerNeg() + Vector(1, 0), Timestamp::fromSeconds(0));
    stp.getIntentAssignment(world);
