...
TEST(PassingEvaluationTest, ratePassShootScore_no_robots_and_facing_directly_away_from_goal){
    ...
    EXPECT_GE(0.1, pass_shoot_score);
}
...
TEST(PassingEvaluationTest, ratePassShootScore_no_open_shot_to_goal){
    ...
    double pass_shoot_score = ratePassShootScore(field, robots_on_field, pass);
    EXPECT_LE(0, pass_shoot_score);
    EXPECT_GE(0.01, pass_shoot_score);
}
...
// As we decrease the open angle to the goal, the shot score should also decrease
    std::vector<Robot> robots_on_field = {
            Robot(0, {0.5,}, {0,0}, Angle::zero(), AngularVelocity::zero(), Timestamp::fromSeconds(0)),
            Robot(0, {0.5,0}, {0,0}, Angle::zero(), AngularVelocity::zero(), Timestamp::fromSeconds(0)),
    };
    Field field = ::Test::TestUtil::createSSLDivBField();
    Pass pass({1,1}, {0,0}, 1, Timestamp::fromSeconds(1));
...
TEST(PassingEvaluationTest, ratePassEnemyRisk_enemy_near_receiver_worse_then_enemy_near_middle_of_pass){
    ...
    double robot_near_receive_point_rating = ratePassEnemyRisk(enemy_team, pass);

    enemy_team.updateRobots({
                                    Robot(0, {5,1}, {0,0}, Angle::zero(), AngularVelocity::zero(), Timestamp::fromSeconds(0))
                            });
    double robot_near_pass_center_rating = ratePassEnemyRisk(enemy_team, pass);

    EXPECT_GE(robot_near_receive_point_rating, robot_near_pass_center_rating);
}
...
TEST(PassingEvaluationTest, calculateInterceptRisk_for_team_several_robots_first_is_close_to_pass){
...
TEST(PassingEvaluationTest, calculateInterceptRisk_for_team_several_robots_last_is_close_to_pass){
...
TEST(PassingEvaluationTest, calculateInterceptRisk_for_robot_sitting_on_pass_trajectory){
    ...
    Robot enemy_robot(0, {5,5}, {0,0}, Angle::zero(), AngularVelocity::zero(), Timestamp::fromSeconds(0));
    Pass pass({0,0}, {10,10}, 3, Timestamp::fromSeconds(1));
...
TEST(PassingEvaluationTest, calculateInterceptRisk_for_robot_just_off_pass_trajectory){
    ...
    Robot enemy_robot(0, {6,7}, {0,0}, Angle::zero(), AngularVelocity::zero(), Timestamp::fromSeconds(0));
    Pass pass({0,0}, {10,10}, 3, Timestamp::fromSeconds(1));
...
TEST(PassingEvaluationTest, calculateInterceptRisk_for_robot_far_away_from_trajectory){
    ...
    Robot enemy_robot(0, {20,-50}, {0,0}, Angle::zero(), AngularVelocity::zero(), Timestamp::fromSeconds(0));
    Pass pass({0,0}, {10,10}, 3, Timestamp::fromSeconds(1));
...
TEST(PassingEvaluationTest, calculateInterceptRisk_robot_at_far_end_of_field){
    ...
    Robot enemy_robot(0, {0,-2}, {0,0}, Angle::zero(), AngularVelocity::zero(), Timestamp::fromSeconds(0));
...
TEST(PassingEvaluationTest, calculateInterceptRisk_enemy_moving_far_away){
    ...
    // intercept it
    Robot enemy_robot(0, {5, -5}, {0,3}, Angle::zero(), AngularVelocity::zero(), Timestamp::fromSeconds(0));
...
TEST(PassingEvaluationTest, calculateInterceptRisk_just_barely_intercept_starting_from_stop){
    ...
    double enemy_travel_distance_from_stop = 0.5 * ENEMY_ROBOT_MAX_ACCELERATION_METERS_PER_SECOND_SQUARED;
    Robot enemy_robot(0, {0, enemy_travel_distance_from_stop}, {0,0}, Angle::zero(), AngularVelocity::zero(), Timestamp::fromSeconds(0));
...
TEST(PassingEvaluationTest, calculateInterceptRisk_just_barely_intercept_starting_with_initial_velocity){
    ...
    double enemy_travel_distance_from_stop = ENEMY_ROBOT_MAX_SPEED_METERS_PER_SECOND + 0.5 * ENEMY_ROBOT_MAX_ACCELERATION_METERS_PER_SECOND_SQUARED;
    Robot enemy_robot(0, {0, enemy_travel_distance_from_stop}, {0,0}, Angle::zero(), AngularVelocity::zero(), Timestamp::fromSeconds(0));
    Pass pass({0,0}, {0,2}, 0.5, Timestamp::fromSeconds(0));
...
TEST(PassingEvaluationTest, ratePassFriendlyCapability_multiple_robots_all_too_far_from_reception_point){
...
TEST(PassingEvaluationTest, ratePassFriendlyCapability_multiple_robots_close_enough_to_reception_point){
