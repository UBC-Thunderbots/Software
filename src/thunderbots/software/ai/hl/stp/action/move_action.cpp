...
MoveAction::MoveAction(double close_to_dest_threshold, Angle close_to_orientation_threshold, bool loop_forever)
    : Action(),
      close_to_dest_threshold(close_to_dest_threshold), close_to_orientation_threshold(close_to_orientation_threshold),
...
void MoveAction::calculateNextIntent(IntentCoroutine::push_type& yield)
{
    ...
    do
    {
        yield(std::make_unique<MoveIntent>(robot->id(), destination, final_orientation,
             ...
             (robot->position() - destination).len() > close_to_dest_threshold || (robot->orientation().minDiff(final_orientation) > close_to_orientation_threshold));
}
