...
namespace Evaluation
{
    ...
    calcBestShotOnGoal(const Field &field, const Team &friendly_team, const Team &enemy_team, bool shoot_on_enemy_goal, const Point &point, double radius, const std::vector<Robot> &robots_to_ignore)
    {
        ...
        if (shoot_on_enemy_goal)
        {
            best_shot = calcBestShotOnGoal(field.enemyGoalpostNeg(),
                                           field.enemyGoalpostPos(), point,
                                           obstacles, radius);
        }
        else
        {
            best_shot = calcBestShotOnGoal(field.friendlyGoalpostPos(),
                                           field.friendlyGoalpostNeg(), point,
                                           obstacles, radius);
        }
    ...
    calcBestShotOnEnemyGoal(const Field &field, const Team &friendly_team, const Team &enemy_team, const Robot &robot, double radius, const std::vector<Robot> &robots_to_ignore)
    {
        ...
        return calcBestShotOnEnemyGoal(field, friendly_team, enemy_team, robot.position(), radius,
    ...
    {
        return calcBestShotOnGoal(field, friendly_team, enemy_team, true, shot_origin, radius, robots_to_ignore);
    }
    ...
    calcBestShotOnFriendlyGoal(const Field &field, const Team &friendly_team, const Team &enemy_team, const Robot &robot, double radius, const std::vector<Robot> &robots_to_ignore)
    {
        ...
        // Ignore the robot shooting the ball
        all_robots_to_ignore.emplace_back(robot);
        return calcBestShotOnFriendlyGoal(field, friendly_team, enemy_team, robot.position(), radius,
                                          all_robots_to_ignore);
    }
    ...
    {
        return calcBestShotOnGoal(field, friendly_team, enemy_team, false, shot_origin, radius, robots_to_ignore);
    }
}  // namespace Evaluation
