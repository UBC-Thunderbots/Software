...
std::optional <std::pair<Point, Duration>> findBestInterceptForBall(Ball ball, Field field, Robot robot) {
    ...
    auto objective_function = [&](std::array<double, 1> x){
        ...
        if(ball.lastUpdateTimestamp() < robot.lastUpdateTimestamp()){
            duration += (robot.lastUpdateTimestamp() - ball.lastUpdateTimestamp()).getSeconds();
        }
    ...
    Duration best_ball_travel_duration = Duration::fromSeconds(std::abs(optimizer.minimize(objective_function, {0}, 50).at(0)));
    ...
    if (robot.lastUpdateTimestamp() > ball.lastUpdateTimestamp()){
        best_ball_travel_duration = best_ball_travel_duration + (robot.lastUpdateTimestamp() - ball.lastUpdateTimestamp());
    }
    ...
    Duration duration_from_robot_timestamp = best_ball_travel_duration +
