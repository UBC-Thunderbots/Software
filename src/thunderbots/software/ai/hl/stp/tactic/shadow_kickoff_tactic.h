...
class ShadowKickoffTactic : public Tactic
{
   public:
     ...
     * to true as the enemy robot position will be constantly updating and will require constant
     * correction.
     */
     ...
     *
     * @param enemy_robot_position The enemy robot to shadow without moving to the enemy side.
     */
    ...
    /*
     * Calculates the point which the robot needes to position it self on the feild to shadow
     * the specified enemy robot. Takes into account the field lines, and does not return points
     * in the centre circle, complying with the rules of the kickoff.
     *
     ...
     * @return The destination point, which is the best point to shadow the specified enemy robot
     */
    Point calculateShadowPoint(const Field& field, const Point& enemy_robot_position);

   private:
    void calculateNextIntent(
