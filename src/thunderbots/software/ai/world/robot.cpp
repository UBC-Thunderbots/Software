...
void Robot::updateState(const Robot &new_robot_data)
{
    ...
    {
        throw std::invalid_argument("Error: Robot updated using a robot with a mismatched id");
    }
    ...
    const std::chrono::milliseconds &milliseconds_in_future) const
{
    if (milliseconds_in_future < std::chrono::milliseconds(0))
    {
        throw std::invalid_argument("Error: Orientation estimate is updating times from the past");
    }
    ...
    const std::chrono::milliseconds &milliseconds_in_future) const
{
    if (milliseconds_in_future < std::chrono::milliseconds(0))
    {
        throw std::invalid_argument("Error: Angular velocity estimate is updating times from the past");
    }
