      ...
      first_draw_call(true)
{
    ...
    // We do this in code rather than from the *.ui file (promoting a custom widget)
    // because the autogenerated code doesn't use absolute include paths and therefore
    // doesn't work with bazel
    auto graphics_view_parent =
        main_widget->ai_visualization_graphics_view->parentWidget();
    delete main_widget->ai_visualization_graphics_view;
    main_widget->ai_visualization_graphics_view =
        new ZoomableQGraphicsView(graphics_view_parent);
