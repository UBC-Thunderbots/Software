...
private:
    /**
     * Creates a new ThunderbotsVisualizer in a new thread and starts running it. We use promises in order for this
     * object to still get pointers to the newly created QApplication and ThunderbotsVisualizer objects so we can
     * control them. These objects must be created in the new thread because the QApplication must be constructed
     * in the thread is will run in, and the ThunderbotsVisualizer must be created in the same context as the
     * QApplication (which in this case is the new thread).
     *
     * @param argc The number of arguments being passed
     * @param argv Keyword arguments for the Visualizer QApplication
     * @param application_promise_ptr A shared_ptr to a QApplication promise that will be set by the new thread
     * @param visualizer_promise_ptr A shared_ptr to a ThunderbotsVisualizer promise that will be set by the new thread
     */
     ...
     * as well as drawing the AI state we want to show, like planned navigator paths.
     */
    void drawAI();
