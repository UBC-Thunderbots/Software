...
VisualizerWrapper::VisualizerWrapper(int argc, char **argv) : ThreadedObserver<World>() {
    auto application_promise = std::make_shared<std::promise<std::shared_ptr<QApplication>>>();
    std::future<std::shared_ptr<QApplication>> application_future = application_promise->get_future();
    auto visualizer_promise = std::make_shared<std::promise<std::shared_ptr<ThunderbotsVisualizer>>>();
    std::future<std::shared_ptr<ThunderbotsVisualizer>> visualizer_future = visualizer_promise->get_future();
...
VisualizerWrapper::~VisualizerWrapper() {
    ...
    QMetaObject::invokeMethod(application.get(), "quit", Qt::ConnectionType::BlockingQueuedConnection);
