...
typedef struct __attribute__((packed)) {
	uint8_t bLength; ///< The length of this descriptor (\c sizeof(usb_dfu_functional_descriptor_t))
	uint8_t bDescriptorType; ///< The type of this descriptor (\ref usb_dfu_dtype_t "USB_DFU_DTYPE_FUNCTIONAL")
	struct __attribute__((packed)) {
		unsigned bitCanDnload : 1; ///< Whether this device can download
		unsigned bitCanUpload : 1; ///< Whether this device can upload
		unsigned bitManifestationTolerant : 1; ///< Whether this device can communicate after manifestation
		unsigned bitWillDetach : 1; ///< Whether this device will detach when sent \c USB_DFU_CREQ_DETACH
		unsigned : 4;
	} bmAttributes; ///< Miscellaneous attributes of the device
	uint16_t wDetachTimeout; ///< How many milliseconds the device waits to start DFU after a detach request
	uint16_t wTransferSize; ///< Maximum size of a single download or upload transfer (not transaction)
 ...
 * \cond HIDDEN
 */
_Static_assert(sizeof(usb_dfu_functional_descriptor_t) == 9, "usb_dfu_functional_descriptor_t wrong size");
