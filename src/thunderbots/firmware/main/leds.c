...
void leds_tick(void) {
	...
	switch (mode) {
		case LEDS_TEST_MODE_NORMAL:
			packet[0U] = 0U;
			packet[1U] = (chicker_auto_armed() ? 0x02U : 0x00U) | (breakbeam_interrupted() ? 0x01U : 0x00U);
			break;
		...
		case LEDS_TEST_MODE_ENCODER:
			...
			for (unsigned int bit = 0U; bit != 2U; ++bit) {
				if (gpio_get_input_raw(*ENCODER_PINS[param & 3U][bit].gpio, ENCODER_PINS[param & 3U][bit].bit)) {
					packet[1U] |= 1U << bit;
				}
			}
			break;
 ...
 * \param[in] param a mode-specific parameter, such as a constant pattern for \ref LEDS_TEST_MODE_CONSTANT or a motor index for \ref LEDS_TEST_MODE_HALL or \ref LEDS_TEST_MODE_ENCODER
 */
