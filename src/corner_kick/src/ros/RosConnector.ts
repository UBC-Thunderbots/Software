...
/**
 * Handles the communication with ROSBridge. Namely handles connecting
...
export class ROSConnector {
  ...
  /**
   * Stores the topics we are currently registered with. This helps us
   * avoids subscribing to a topic we are already subscribed with.
   */
  private topics: string[] = [];

  /**
   * Stores the callback functions for each topic. When a new message arrives
   * for a given topic, we call the functions stored in
   * this dictionary.
   */
  ...
  /**
   * Internal method to subscribe to all ROS topics. This is the method that actually
   * "subscribes" in a ROS sense. We do this to create an abstraction
  ...
  private subscribeToAllTopics = () => {
    ...
    setTimeout(this.subscribeToAllTopics, 10000); // TODO Make 10000 configurable
  };
   ...
   */
  private onTopicsReceived = (response: {
    topics: string[];
    types: string[];
  }) => {
  ...
  private onMessageReceived = (name: string) => (message: any) => {
    if (this.topicsSubscriptions[name] !== undefined) {
      ...
      console.error(
        'Received message but we did not have a subscription for it',
      );
    }
  };
}
