...
INSTANTIATE_TEST_CASE_P(encode_decode_test, CobsEncodingTest,
    ::testing::Values(
        std::make_tuple(
                std::vector<uint8_t>({0x01}),
                std::vector<uint8_t>({START_END_FLAG_BYTE, 0x02, 0x01, START_END_FLAG_BYTE})),
        std::make_tuple(
                std::vector<uint8_t>({START_END_FLAG_BYTE}), std::vector<uint8_t>({START_END_FLAG_BYTE, 0x01, 0x01, START_END_FLAG_BYTE})),
        std::make_tuple(std::vector<uint8_t>({0x01, 0x02, 0x03, 0x04}), std::vector<uint8_t>({START_END_FLAG_BYTE, 0x05, 0x01, 0x02, 0x03, 0x04, START_END_FLAG_BYTE})),
        std::make_tuple(std::vector<uint8_t>({START_END_FLAG_BYTE, START_END_FLAG_BYTE, START_END_FLAG_BYTE, START_END_FLAG_BYTE}), std::vector<uint8_t>({START_END_FLAG_BYTE, 0x01, 0x01, 0x01, 0x01, 0x01, START_END_FLAG_BYTE})),
        std::make_tuple(generateBigByteVectorToEncode(), generateBigByteVectorToDecode())
    ));

class CobsDecodingErrorTest
...
TEST_F(UartFramingTest, verify_length_and_crc_detect_wrong_length_and_crc_test) {
    UartMessageFrame<TestMessage> test_frame = {sizeof(test_message), TEST_MESSAGE_CRC, test_message};
    EXPECT_TRUE(verifyLengthAndCrc(test_frame));
    test_frame.length = 0;
...
TEST_F(UartFramingTest, verify_length_and_crc_detect_byte_error_test) {
    ...
    std::vector<uint8_t> bytes (test_message_ptr, test_message_ptr + sizeof(test_message_ptr));
    // Add byte error
    bytes[2] = 0xFF;
    //Convert back to TestMessage
    TestMessage test_message_byte_error;
    std::copy(bytes.begin(), bytes.end(), reinterpret_cast<uint8_t*>(&test_message_byte_error));
    UartMessageFrame<TestMessage> test_frame = {sizeof(test_message), TEST_MESSAGE_CRC, test_message_byte_error};
    EXPECT_FALSE(verifyLengthAndCrc(test_frame));
}
...
TEST_F(UartFramingTest, marshalling_test) {
    ...
    EXPECT_EQ(bytes.size(), sizeof(test_message) + 2 * sizeof(uint16_t) + 3 * sizeof(uint8_t));
