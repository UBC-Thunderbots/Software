...
namespace aabb
{
    ...
    Tree::Tree(unsigned int dimension_, double skinThickness_, unsigned int nParticles,
    ...
    Tree::Tree(unsigned int dimension_, double skinThickness_,
                              ...
                              double radius)
    {
        ...
        particleMap.insert(
            std::unordered_map<unsigned int, unsigned int>::value_type(particle, node));

        // Store the particle index.
        nodes[node].particle = particle;
    }

    void Tree::insertParticle(unsigned int particle, std::vector<double>& lowerBound,
