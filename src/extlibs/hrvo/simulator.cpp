...
HRVOSimulator::HRVOSimulator(float time_step, const RobotConstants_t &robot_constants)
    : global_time(0.0f),
      last_update_friendlies_timestamp(Timestamp::fromSeconds(0.0)),
      time_step(time_step),
...
// TODO: Update to 2021 robot constants: simulated_er_force_sim_text_fixture.cpp and robot.h
// TODO: Add CallGrind Artifacts to GitHub Actions as job on Push!?
// TODO: Replace friendly_robot_id_map (robot_id --> agent_id) with (robot_id --> shared_ptr<Agent>) and remove
//       all use cases of AgentNo/agent_id
// NOTE: Robot doesn't stop right at destination at times, since the robotReachedDestination transition condition
...
void HRVOSimulator::updateWorld(const World &world)
{
    ...
    {
        // Update Agents
        if (world.getMostRecentTimestamp() >= last_update_friendlies_timestamp + Duration::fromSeconds(0.5))
        {
            // HRVO Simulator has a more accurate estimate of the current friendly robot state
            // than the world generated by SensorFusion, as a result, the robot state is only
            // updated every 0.5sec to avoid HRVO Simulator deviating from the real world.
            last_update_friendlies_timestamp = world.getMostRecentTimestamp();
            for (const Robot &friendly_robot : friendly_team)
            {
