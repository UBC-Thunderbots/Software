...
void HRVOSimulator::updateWorld(const World &world)
{
    ...
    for (const Robot &enemy_robot : enemy_team)
    {
        ...
        if (agent_index_iter != enemy_robot_id_map.end() &&
            agents[agent_index_iter->second].has_value())
        {
            agents[agent_index_iter->second].value()->setPosition(
                enemy_robot.position().toVector());
            agents[agent_index_iter->second].value()->setVelocity(enemy_robot.velocity());
        }
        else
        {
            // we arbitrarily choose a destination 5 seconds away at the currently
            // velocity
            Vector destination =
                                        ...
                                        AgentType type)
{
    ...
    {
        agents[agent_index_iter->second] =
...
void HRVOSimulator::visualize(unsigned int robot_id) const
{
    ...
    auto vo_protos =
        ((HRVOAgent) * (friendly_agent_opt.value())).getVelocityObstaclesAsProto();
    ...
    unsigned int robot_id) const
{
        ...
        agents[agent_index_iter->second].has_value())
    {
        ...
        auto hrvo_agent =
            std::static_pointer_cast<HRVOAgent>(agents[agent_index].value());
        return hrvo_agent;
    }
    return std::nullopt;
}
                  ...
                  [&world](std::optional<std::shared_ptr<Agent>> &agent) {
                      ...
                      auto team_iter = std::find_if(
                          ...
                          [&agent](const Robot &robot) {
                              return (robot.id() == agent.value()->getRobotId());
                          });
