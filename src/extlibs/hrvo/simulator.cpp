...
HRVOSimulator::HRVOSimulator(float time_step, const RobotConstants_t &robot_constants)
    : global_time(0.0f),
      ...
      update_positions_frequency(Duration::fromSeconds(0.3)),
      time_step(time_step),
...
// shared_ptr<Agent>) and remove all use cases of AgentNo/agent_id
// TODO: Double check HRVO Visualization. VOs sometimes seem off and incorrect
    ...
    for (auto &[robot_id, primitive] : primitive_set.robot_primitives())
    {
        ...
        if (hrvo_agent_opt.has_value())
        {
            ...
            if (primitive.has_move())
            {
                ...
                float path_radius =
                        (hrvo_agent->getMaxSpeed() * time_step) / 2 * DESTINATION_THRESHOLD_SCALE;
                path = AgentPath(
...
std::size_t HRVOSimulator::addHRVORobotAgent(const Robot &robot)
{
    ...
    const float radius = ROBOT_MAX_RADIUS_METERS * ROBOT_AGENT_RADIUS_SCALE;
    Vector velocity;
    ...
    // Max distance which the robot can travel in one time step + scaling
    float path_radius        = (max_speed * time_step) / 2 * DESTINATION_THRESHOLD_SCALE;
    float uncertainty_offset = 0.f;

    AgentPath path =
        AgentPath({PathPoint(destination_point, speed_at_goal)}, path_radius);

    return addHRVOAgent(position, radius, velocity, max_speed,
                                                       ...
                                                       const Vector &destination)
{
    ...
    return addLinearVelocityAgent(position, radius, velocity, max_speed,
                                  max_accel, path);
}
