...
void KdTree::buildRecursive(std::size_t begin, std::size_t end, std::size_t node)
{
    ...
    if (end - begin > HRVO_MAX_LEAF_SIZE)
    {
        ...
        while (true)
        {
            while (left <= right &&
                   (vertical
                        ? simulator_->agents_[agents_[left]]->getPosition().getX()
                        : simulator_->agents_[agents_[left]]->getPosition().getY()) < split)
            {
                ++left;
            }
            ...
            while (right >= left &&
                   (vertical
                        ...
                        : simulator_->agents_[agents_[right]]->getPosition().getY()) >= split)
            {
                --right;
            }
