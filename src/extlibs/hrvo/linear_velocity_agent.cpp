...
LinearVelocityAgent::LinearVelocityAgent(HRVOSimulator *simulator, const Vector &position, float radius,
                                         float max_radius_inflation, const Vector &velocity, float maxSpeed,
                                         float maxAccel, AgentPath &path)
    : Agent(simulator, position, radius, max_radius_inflation, velocity, velocity, maxSpeed, maxAccel, path)
{
}
