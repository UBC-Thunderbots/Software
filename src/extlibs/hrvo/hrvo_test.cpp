...
class HRVOTest : public ::testing::Test
{
   public:
        ...
        for (float x = -(field_width / 2); x <= (field_width / 2); x += robot_offsets)
        {
            for (float y : {-(field_height / 2), field_height / 2})
            {
                ...
                simulator.addAgent(position, simulator.addGoal(position), 1.f, 1, 0.25f,
                                   0.25f, 0.1f, 0.1f, 0.f, 0.1f, Vector(), 0.f);
            }
        }
    ...
    void add_static_obstacle(const Vector position, const float radius)
    {
        simulator.addAgent(position, simulator.addGoal(position), 1.f, 1, radius, radius,
                           0.1f, 0.1f, 0.f, 0.1f, Vector(), 0.f);
    }
...
{
    const Vector goal_offset  = Vector(8.f, 0);
    const Vector robot_offset = Vector(0.f, -ROBOT_RADIUS * 2.5f);
    for (std::size_t i = 0; i < 1; ++i)
    {
        const Vector position = -1 * (goal_offset / 2) + Vector(0.f, 2.5f) +
                                (static_cast<float>(i) * robot_offset);
        simulator.addAgent(position, simulator.addGoal(position + goal_offset));
    }
    add_static_obstacle(Vector(0, 2.f), 0.75f);
    create_div_b_field();
}
...
TEST_F(HRVOTest, 25_robots_around_circle)
{
    ...
    simulator.addAgent(Vector(0.f, 0.f), simulator.addGoal(Vector(0.f, 0.f)));
    for (std::size_t i = 0; i < num_robots; ++i)
    {
        ...
        const Vector position = circle_radius * Vector(x, y);
...
TEST_F(HRVOTest, 5_robots_in_vertical_line)
{
    ...
    const Vector goal_offset  = Vector(0.f, -6.f);
    const Vector robot_offset = Vector(0.f, -ROBOT_RADIUS * 2.5f);
    for (std::size_t i = 0; i < num_robots; ++i)
    {
        const Vector position = static_cast<float>(i) * robot_offset;
        simulator.addAgent(position, simulator.addGoal(position + goal_offset));
    }
}
...
{
    simulator.addAgent(Vector(-4.f, 0.f), simulator.addGoal(Vector(4.f, 0.f)));
}

TEST_F(HRVOTest, 1_robot_two_goals)
{
    simulator.addAgent(Vector(-4.f, 0.f),
                       simulator.addGoalPositions({Vector(4.f, 0.f), Vector(-4.f, 0.f)}));
}

TEST_F(HRVOTest, 1_robot_moving_in_square)
{
    simulator.addAgent(Vector(-4.f, -4.f), simulator.addGoalPositions(
                                               {Vector(4.f, -4.f), Vector(4.f, 4.f),
                                                Vector(-4.f, 4.f), Vector(-4.f, -4.f)}));
}

TEST_F(HRVOTest, 1_robot_moving_in_square_with_final_speed)
{
    simulator.addAgent(Vector(-4.f, -4.f),
                       simulator.addGoalPositions({Vector(4.f, -4.f), Vector(4.f, 4.f),
                                                   Vector(-4.f, 4.f), Vector(-4.f, -4.f)},
                                                  {2.f, 2.f, 2.f, 0.f}));
}
