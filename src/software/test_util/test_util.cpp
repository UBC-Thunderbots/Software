...
World TestUtil::setFriendlyRobotPositions(World world, std::vector<Point> robot_positions,
                                          const Timestamp &timestamp)
{
...
World TestUtil::setEnemyRobotPositions(World world, std::vector<Point> robot_positions,
                                       const Timestamp &timestamp)
{
    Team new_enemy_team =
...
World TestUtil::setBallVelocity(World world, Vector ball_velocity, Timestamp timestamp)
{
    TimestampedBallState ballState =
...
::testing::AssertionResult TestUtil::equalWithinTolerance(const RobotState &state1,
                                                          ...
                                                          const Angle &angular_tolerance)
{
        ...
        !orientation_equality_result || !angular_velocity_equality_result)
    {
        ...
        {
            assertion_result << std::endl
                             << "The first state's orientation was "
                             << state1.orientation()
                             << ", the second state's orientation was "
                             << state2.orientation();
        }
...
::testing::AssertionResult TestUtil::equalWithinTolerance(const RobotStateWithId &state1,
