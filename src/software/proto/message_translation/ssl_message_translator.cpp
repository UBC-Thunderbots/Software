...
std::unique_ptr<SSL_FieldLineSegment> createFieldLineSegment(const Segment& segment, const float thickness, const std::string& name, const SSL_FieldShapeType& shape_type) {
    if(thickness < 0) {
...
std::unique_ptr<SSL_FieldCircularArc> createFieldCircularArc(const Circle& circle, float thickness, const std::string& name, const SSL_FieldShapeType& shape_type) {
    if(thickness < 0) {
...
std::unique_ptr<SSL_GeometryFieldSize> createGeometryFieldSize(const Field& field, const float thickness) {
    ...
    geometry_field_size->set_penalty_area_depth(static_cast<int32_t>(field.defenseAreaXLength() * MILLIMETERS_PER_METER));
    geometry_field_size->set_penalty_area_width(static_cast<int32_t>(field.defenseAreaYLength() * MILLIMETERS_PER_METER));
    ...
    *line = *(createFieldLineSegment(top_touch_line, thickness, ssl_field_line_names.at(SSLFieldLines::TOP_TOUCH_LINE), SSL_FieldShapeType::TopTouchLine).release());
    ...
    *line = *(createFieldLineSegment(bottom_touch_line, thickness, ssl_field_line_names.at(SSLFieldLines::BOTTOM_TOUCH_LINE), SSL_FieldShapeType::BottomTouchLine).release());

    line = geometry_field_size->add_field_lines();
    Segment left_goal_line(field.fieldLines().negXPosYCorner(), field.fieldLines().negXNegYCorner());
    *line = *(createFieldLineSegment(left_goal_line, thickness, ssl_field_line_names.at(SSLFieldLines::LEFT_GOAL_LINE), SSL_FieldShapeType::LeftGoalLine).release());

    line = geometry_field_size->add_field_lines();
    Segment right_goal_line(field.fieldLines().posXPosYCorner(), field.fieldLines().posXNegYCorner());
    *line = *(createFieldLineSegment(right_goal_line, thickness, ssl_field_line_names.at(SSLFieldLines::RIGHT_GOAL_LINE), SSL_FieldShapeType::RightGoalLine).release());

    line = geometry_field_size->add_field_lines();
    Segment halfway_line = field.halfwayLine();
    *line = *(createFieldLineSegment(halfway_line, thickness, ssl_field_line_names.at(SSLFieldLines::HALFWAY_LINE), SSL_FieldShapeType::HalfwayLine).release());

    line = geometry_field_size->add_field_lines();
    Segment center_line = Segment(field.friendlyGoalCenter(), field.enemyGoalCenter());
    *line = *(createFieldLineSegment(center_line, thickness, ssl_field_line_names.at(SSLFieldLines::CENTER_LINE), SSL_FieldShapeType::CenterLine).release());

    line = geometry_field_size->add_field_lines();
    Segment left_penalty_stretch = Segment(field.friendlyDefenseArea().posXNegYCorner(), field.friendlyDefenseArea().posXPosYCorner());
    *line = *(createFieldLineSegment(left_penalty_stretch, thickness, ssl_field_line_names.at(SSLFieldLines::LEFT_PENALTY_STRETCH), SSL_FieldShapeType::LeftPenaltyStretch).release());

    line = geometry_field_size->add_field_lines();
    Segment right_penalty_stretch = Segment(field.enemyDefenseArea().negXNegYCorner(), field.enemyDefenseArea().negXPosYCorner());
    *line = *(createFieldLineSegment(right_penalty_stretch, thickness, ssl_field_line_names.at(SSLFieldLines::RIGHT_PENALTY_STRETCH), SSL_FieldShapeType::RightPenaltyStretch).release());

    line = geometry_field_size->add_field_lines();
    Segment right_goal_top_line = Segment(field.enemyGoal().negXPosYCorner(), field.enemyGoal().posXPosYCorner());
    *line = *(createFieldLineSegment(right_goal_top_line, thickness, ssl_field_line_names.at(SSLFieldLines::RIGHT_GOAL_TOP_LINE), SSL_FieldShapeType::Undefined).release());

    line = geometry_field_size->add_field_lines();
    Segment right_goal_bottom_line = Segment(field.enemyGoal().negXNegYCorner(), field.enemyGoal().posXNegYCorner());
    *line = *(createFieldLineSegment(right_goal_bottom_line, thickness, ssl_field_line_names.at(SSLFieldLines::RIGHT_GOAL_BOTTOM_LINE), SSL_FieldShapeType::Undefined).release());

    line = geometry_field_size->add_field_lines();
    Segment right_goal_depth_line = Segment(field.enemyGoal().posXPosYCorner(), field.enemyGoal().posXNegYCorner());
    *line = *(createFieldLineSegment(right_goal_depth_line, thickness, ssl_field_line_names.at(SSLFieldLines::RIGHT_GOAL_DEPTH_LINE), SSL_FieldShapeType::Undefined).release());

    line = geometry_field_size->add_field_lines();
    Segment left_goal_top_line = Segment(field.friendlyGoal().negXPosYCorner(), field.friendlyGoal().posXPosYCorner());
    *line = *(createFieldLineSegment(left_goal_top_line, thickness, ssl_field_line_names.at(SSLFieldLines::LEFT_GOAL_TOP_LINE), SSL_FieldShapeType::Undefined).release());

    line = geometry_field_size->add_field_lines();
    Segment left_goal_bottom_line = Segment(field.friendlyGoal().negXNegYCorner(), field.friendlyGoal().posXNegYCorner());
    *line = *(createFieldLineSegment(left_goal_bottom_line, thickness, ssl_field_line_names.at(SSLFieldLines::LEFT_GOAL_BOTTOM_LINE), SSL_FieldShapeType::Undefined).release());

    line = geometry_field_size->add_field_lines();
    Segment left_goal_depth_line = Segment(field.friendlyGoal().negXPosYCorner(), field.friendlyGoal().negXNegYCorner());
    *line = *(createFieldLineSegment(left_goal_depth_line, thickness, ssl_field_line_names.at(SSLFieldLines::LEFT_GOAL_DEPTH_LINE), SSL_FieldShapeType::Undefined).release());
    ...
    *line = *(createFieldLineSegment(left_field_left_penalty_stretch, thickness, ssl_field_line_names.at(SSLFieldLines::LEFT_FIELD_LEFT_PENALTY_STRETCH), SSL_FieldShapeType::LeftFieldLeftPenaltyStretch).release());
    ...
    *line = *(createFieldLineSegment(left_field_right_penalty_stretch, thickness, ssl_field_line_names.at(SSLFieldLines::LEFT_FIELD_RIGHT_PENALTY_STRETCH), SSL_FieldShapeType::LeftFieldRightPenaltyStretch).release());
    ...
    *line = *(createFieldLineSegment(right_field_left_penalty_stretch, thickness, ssl_field_line_names.at(SSLFieldLines::RIGHT_FIELD_LEFT_PENALTY_STRETCH), SSL_FieldShapeType::RightFieldLeftPenaltyStretch).release());
    ...
    *line = *(createFieldLineSegment(right_field_right_penalty_stretch, thickness, ssl_field_line_names.at(SSLFieldLines::RIGHT_FIELD_RIGHT_PENALTY_STRETCH), SSL_FieldShapeType::RightFieldRightPenaltyStretch).release());

    auto arc = geometry_field_size->add_field_arcs();
    Circle center_circle = field.centerCircle();
    *arc = *(createFieldCircularArc(center_circle, thickness, ssl_circular_arc_names.at(SSLCircularArcs::CENTER_CIRCLE), SSL_FieldShapeType::CenterCircle).release());
