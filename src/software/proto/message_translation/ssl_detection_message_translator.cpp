...
std::unique_ptr<SSL_DetectionBall> createDetectionBall(const BallState& ball) {
    ...
    // We don't have the information to simulate a camera and therefore the area of the ball
...
std::unique_ptr<SSL_DetectionRobot> createDetectionRobot(const RobotStateWithId& robot) {
    ...
    auto x_position_mm = static_cast<float>(robot.robot_state.position().x() * MILLIMETERS_PER_METER);
    auto y_position_mm = static_cast<float>(robot.robot_state.position().y() * MILLIMETERS_PER_METER);
    detection_robot->set_x(x_position_mm);
    detection_robot->set_y(y_position_mm);
    detection_robot->set_orientation(static_cast<float>(robot.robot_state.orientation().toRadians()));
    ...
    detection_robot->set_height(static_cast<float>(robot.robot_state.height() * MILLIMETERS_PER_METER));

    return std::move(detection_robot);
}

std::unique_ptr<SSL_DetectionFrame> createDetectionFrame(const uint32_t camera_id, const Timestamp& t_capture, const uint32_t frame_number, const std::vector<BallState>& balls, const std::vector<RobotStateWithId>& yellow_robots, const std::vector<RobotStateWithId>& blue_robots) {
