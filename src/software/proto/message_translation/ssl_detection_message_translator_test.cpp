...
TEST(SSLDetectionMessageTranslatorTest, test_create_detection_ball) {
    ...
    auto detection_ball = createDetectionBall(ball_state);
...
TEST(SSLDetectionMessageTranslatorTest, test_create_detection_robot) {
    const RobotState state(Point(0.0, -0.5), Vector(1, 2), Angle::quarter(), AngularVelocity::threeQuarter(), 0.15);
    const RobotStateWithId state_with_id{.id = 2, .robot_state = state};

    auto detection_robot = createDetectionRobot(state_with_id);
...
TEST(SSLDetectionMessageTranslatorTest, test_create_detection_frame) {
    ...
    auto detection_frame = createDetectionFrame(camera_id, t_capture, frame_number, {ball_state}, yellow_robot_states, blue_robot_states);
...
TEST(SSLDetectionMessageTranslatorTest, test_convert_ball_state_to_proto_and_back) {
    ...
    auto detection_frame = createDetectionFrame(camera_id, t_capture, frame_number, {ball_state}, {}, {});
    ASSERT_TRUE(detection_frame);

    std::vector<BallDetection> ball_detections = SSLProtobufReader().getBallDetections({*detection_frame});
