   ...
   protected:
                          ...
                          unsigned int num_steps_to_ignore = 0)
    {
        ...
        for (unsigned i = 0; i < num_iterations; i++)
        {
            ...
            if(filtered_ball->velocity().length() < ball_filter.BALL_MIN_SPEED_FOR_ROLLING_ACCELERATION) {
                EXPECT_EQ(acceleration, Vector());
            } else {
                auto mag_difference = acceleration.length() - ROLLING_FRICTION_ACCELERATION_MAGNITUDE;
                EXPECT_LT(mag_difference, ROLLING_FRICTION_ACCELERATION_MAGNITUDE);
                EXPECT_EQ(acceleration.orientation(), filtered_ball->velocity().rotate(Angle::half()).orientation());
            }
        }
    }
