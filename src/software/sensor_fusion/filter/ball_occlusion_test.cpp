...
{
    protected: Field field = Field::createSSLDivisionBField(); 
};
...
TEST_F(BallOcclusionTest, simulate_shot_from_friendly_right_to_friendly)
{
    ...
    auto friendly_robots = TestUtil::createStationaryRobotStatesWithId({Point(-4.5, 0)});
    setFriendlyGoalie(0);
    auto enemy_robots = TestUtil::createStationaryRobotStatesWithId({Point(-3.5+0.2/sqrt(2.0), -1-0.2/sqrt(2.0))});
    setEnemyGoalie(0);
    setAIPlay(TYPENAME(HaltPlay));
    setRefereeCommand(RefereeCommand::HALT, RefereeCommand::HALT);
    std::vector<ValidationFunction> terminating_validating_function = {};
...
{
    BallState ball_state(Point(3.5, -1), Vector(1, 1));
    auto friendly_robots = TestUtil::createStationaryRobotStatesWithId({Point(3.5-0.2/sqrt(2.0), -1-0.2/sqrt(2.0))});
    setFriendlyGoalie(0);
    auto enemy_robots = TestUtil::createStationaryRobotStatesWithId({Point(4.5, 0)});
    setEnemyGoalie(0);
    setAIPlay(TYPENAME(HaltPlay));
...
{
    BallState ball_state(Point(3.5, 1), Vector(1, -1));
    auto friendly_robots = TestUtil::createStationaryRobotStatesWithId({Point(3.5-0.2/sqrt(2), 1+0.2/sqrt(2))});
    setFriendlyGoalie(0);
    auto enemy_robots = TestUtil::createStationaryRobotStatesWithId({Point(4.5, 0)});
    setEnemyGoalie(0);
    setAIPlay(TYPENAME(HaltPlay));
...
{
    BallState ball_state(Point(-4.5+0.2/sqrt(2.0), 3-0.2/sqrt(2.0)), Vector(3.0/sqrt(13.0), -2.0/sqrt(13.0)));
    auto friendly_robots = TestUtil::createStationaryRobotStatesWithId({Point(-4.5, 3), Point(-1.5, 1)});
    setFriendlyGoalie(0);
    auto enemy_robots = TestUtil::createStationaryRobotStatesWithId({Point(4.5, 0)});
    setEnemyGoalie(0);
    setAIPlay(TYPENAME(HaltPlay));
...
/* one test for horizontal and vertical on one half (vertical or horizontal) should be representative*/
