...
TEST_F(RobotFilterTest, no_match_robot_data_robot_state_expired_test)
{
    ...
    new_robot_data = {{1, Point(2, 0), Angle::fromRadians(1), 0.5, Timestamp::fromSeconds(0.1)}};
    ...
    new_robot_data = {
        {2, Point(2, 0), Angle::fromRadians(1), 0.5, Timestamp::fromSeconds(EXPIRED_FRAME_THRESHOLD_TEST + 2)}};
    EXPECT_EQ(std::nullopt,
              robot_filter.estimateRobotState(new_robot_data, Timestamp::fromSeconds(EXPIRED_FRAME_THRESHOLD_TEST + 2.1)));
}
...
TEST_F(RobotFilterTest, no_match_robot_data_robot_state_not_expired_test)
{
    ...
    new_robot_data = {
        {2, Point(2, 0), Angle::fromRadians(1), 0.5, Timestamp::fromSeconds(EXPIRED_FRAME_THRESHOLD_TEST + 2)}};
    std::optional<Robot> result = robot_filter.estimateRobotState(new_robot_data, Timestamp::fromSeconds(EXPIRED_FRAME_THRESHOLD_TEST + 2.1));

    // Result isn't Optional
    ASSERT_TRUE(result.has_value());
    // test
    EXPECT_EQ(result->id(), 1);
    EXPECT_EQ(result->timestamp(), Timestamp::fromSeconds(EXPIRED_FRAME_THRESHOLD_TEST + 2.1));
}
...
TEST_F(RobotFilterTest, two_match_robot_data_robot_state_not_expired_test)
{
    ...
    std::optional<Robot> result = robot_filter.estimateRobotState(new_robot_data, Timestamp::fromSeconds(10));

    // Result isn't Optional
    ASSERT_TRUE(result.has_value());
    // Test that the Orientation went towards 1.25, since the second one has higher confidence.
...
TEST_F(RobotFilterTest, large_orientation_angle_wrapping_test)
{
    ...
    // Give it 1 data point so that it doesn't break due to "prev_" variables
    new_robot_data = {{1, Point(2, 0), Angle::fromRadians(0), 0.5, Timestamp::fromSeconds(0.1)}};
    robot_filter.estimateRobotState(new_robot_data, Timestamp::fromSeconds(0.2));

    // make it rotate a lot
    new_robot_data = {{1, Point(2, 0), Angle::fromRadians(M_PI * 2 - 0.2), 0.5, Timestamp::fromSeconds(3)}};
    ...
    new_robot_data = {{2, Point(2, 0), Angle::fromRadians(M_PI * 2 - 0.2), 0.5, Timestamp::fromSeconds(3)}};
    std::optional<Robot> result = robot_filter.estimateRobotState(new_robot_data, Timestamp::fromSeconds(3.6));
