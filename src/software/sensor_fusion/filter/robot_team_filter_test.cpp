...
TEST(RobotTeamFilterTest, one_robot_detection_update_test)
{
    ...
    EXPECT_EQ(robot_detection.orientation, robots[0].currentState().robotState().orientation());
...
TEST(RobotTeamFilterTest, detections_with_same_timestamp_test)
{
    ...
    for (unsigned int i = 0; i < num_robots; i++)
    {
        ...
        EXPECT_EQ(robot_detections[i].position, robot.currentState().robotState().position());
        EXPECT_EQ(robot_detections[i].orientation, robot.currentState().robotState().orientation());
