...
class KalmanFilter
{
   public:
    ...
    void update(Eigen::Matrix<double, dim_z, 1> z)
    {
        ...
        Eigen::Matrix<double, dim_z, dim_z> newSum =
            sum.unaryExpr([](double l) { return (fabs(l) < 1.0e-20) ? 0. : l; });
        Eigen::Matrix<double, dim_x, dim_z> K =
            P *
            (H.transpose() *
        ...
        Eigen::Matrix<double, dim_x, dim_x> posteriorCov =
            Eigen::Matrix<double, dim_x, dim_x>::Identity() - K * H;
