...
TEST_F(RobotTest, update_state_with_all_params)
{
    ...
    robot.updateState(RobotStateWithTimestamp(Point(-1.2, 3), Vector(2.2, -0.05), Angle::quarter(),
...
TEST_F(RobotTest, update_state_with_new_robot)
{
    ...
    RobotStateWithTimestamp update_robot =
        RobotStateWithTimestamp(Point(-1.2, 3), robot.velocity(), Angle::quarter(),
...
TEST_F(RobotTest, get_position_history)
{
    ...
    robot.updateState(RobotStateWithTimestamp(Point(-1.2, 3), Vector(2.2, -0.05), Angle::quarter(),
                                              AngularVelocity::fromRadians(1.1), half_second_future));
    robot.updateState(RobotStateWithTimestamp(Point(-1.3, 3), Vector(2.3, -0.05), Angle::quarter(),
                                              AngularVelocity::fromRadians(1.1), half_second_future));

    boost::circular_buffer<RobotStateWithTimestamp> previous_states = robot.getPreviousStates();
...
TEST_F(RobotTest, get_orientation_history)
{
    ...
    for (int i = 0; i < previous_states.size(); i++)
    {
        previous_orientations.push_back(previous_states.at(i).orientation());
    }
...
TEST_F(RobotTest, get_angular_velocity_history)
{
    ...
    for (int i = 0; i < previous_states.size(); i++)
    {
        previous_angular_velocities.push_back(previous_states.at(i).angularVelocity());
    }
