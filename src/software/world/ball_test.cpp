...
TEST_F(BallTest, update_state_with_all_params)
{
    ...
    ball.updateCurrentState(
        BallState(Point(-4.23, 1.07), Vector(1, 2), one_second_future));
...
TEST_F(BallTest, update_state_with_new_position_old_velocity)
{
    ...
    ball.updateCurrentState(BallState(Point(0.01, -99.8), ball.velocity(), current_time));
...
TEST_F(BallTest, update_state_with_new_velocity_old_position)
{
    ...
    ball.updateCurrentState(
        BallState(ball.position(), Vector(-0.0, -9.433), current_time));
...
TEST_F(BallTest, update_state_with_new_ball)
{
    ...
    ball.updateCurrentState(ball_updated_state);
...
TEST_F(BallTest, update_state_with_past_timestamp)
{
    ...
    ASSERT_THROW(ball.updateCurrentState(
                     BallState(Point(-4.23, 1.07), Vector(1, 2), one_second_past)),
...
TEST_F(BallTest, update_state_to_predicted_state_with_future_timestamp)
{
    ...
    EXPECT_TRUE(Point(-4.0717, -0.1086).isClose(Point(ball.velocity()), EPSILON));
...
TEST_F(BallTest, get_velocity_at_future_time_with_positive_ball_velocity)
{
    ...
    EXPECT_TRUE(Point(0.9851, 1.9702)
                    .isClose(Point(ball.estimateVelocityAtFutureTime(
                                 Duration::fromMilliseconds(150))),
                             EPSILON));
    EXPECT_TRUE(Point(0.9048, 1.8097)
                    .isClose(Point(ball.estimateVelocityAtFutureTime(
                                 Duration::fromMilliseconds(1000))),
                             EPSILON));
    EXPECT_TRUE(Point(0.8187, 1.6375)
                    .isClose(Point(ball.estimateVelocityAtFutureTime(
...
TEST_F(BallTest, get_velocity_at_future_time_with_negative_ball_velocity)
{
    ...
    EXPECT_TRUE(Point(-4.4330, -0.1182)
                    .isClose(Point(ball.estimateVelocityAtFutureTime(
                                 Duration::fromMilliseconds(150))),
                             EPSILON));
    EXPECT_TRUE(Point(-4.0717, -0.1086)
                    .isClose(Point(ball.estimateVelocityAtFutureTime(
                                 Duration::fromMilliseconds(1000))),
                             EPSILON));
    EXPECT_TRUE(Point(-3.6843, -0.0982)
                    .isClose(Point(ball.estimateVelocityAtFutureTime(
                                 Duration::fromMilliseconds(2000))),
                             EPSILON));
}
...
TEST_F(BallTest, get_position_history)
{
    ...
    ball.updateCurrentState(
        BallState(Point(-1.2, 3), Vector(2.2, -0.05), half_second_future));
    ball.updateCurrentState(
...
TEST_F(BallTest, get_timestamp_index_fetches_first_index)
{
    ...
    ball.updateCurrentState(
        BallState(Point(-1.2, 3), Vector(2.2, -0.05), half_second_future));
    ball.updateCurrentState(
        BallState(Point(-1.3, 3), Vector(2.3, -0.05), one_second_future));
