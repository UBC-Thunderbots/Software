...
class HomeScreen(Screen):
    def __init__(self, lcd_display, redis_dict, status_codes):
        ...
        def draw_screen():
            ...
            self.lcd_display.draw.text((x, y), robot_id_str, font=self.font, fill="#FFFFFF")
            
            x += self.font.getsize(robot_id_str)[0] + cursor_size + PADDING 
            channel_id_str = "Channel ID"
            self.lcd_display.draw.text((x, y), channel_id_str, font=self.font, fill="#FFFFFF") 
            
            y += self.font.getsize(channel_id_str)[1]
            x += self.font.getsize(channel_id_str)[0]/2
            self.lcd_display.draw.text(
                (x, y), str(int(self.ids[CHANNEL_ID])), font=self.big_font, fill="#00ffff"
            )
            ...
            self.lcd_display.draw.text((x, y), battery_str, font=self.font, fill="#FFFFFF")
            x += (self.font.getsize(battery_str))[0]
            self.lcd_display.draw.text(
                (x, 48+12), str(round(self.battery_voltage, 1)), font=self.font, fill="#00ffff"
            )
            ...
            x = cursor_size
            y += self.font_size
            self.lcd_display.draw.text((x, y), packet_loss_str, font=self.font, fill="#FFFFFF")
            x += (self.font.getsize(packet_loss_str))[0]
            self.lcd_display.draw.text(
                (x, y), str(round(self.packet_loss, 1)), font=self.font, fill="#00ffff"
            )
            
            self.lcd_display.draw.text((cursor_size, self.lcd_display.height - PADDING - self.font_size), "Go to Menu screen", font=self.font, fill="#FFFFFF")
            ...
            self.lcd_display.draw.text(
                (x, y), ">", font=self.font, fill="#ffffff"
            )
