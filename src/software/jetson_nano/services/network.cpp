...
float calculatePacketLossRate(std::queue<uint64_t> recent_proto_seq_nums,
                              uint64_t seq_num)
{
...
void NetworkService::primitiveSetCallback(TbotsProto::PrimitiveSet input)
{
    ...
    float packet_loss_rate =
        calculatePacketLossRate(recent_primitive_set_seq_nums, seq_num);
...
void NetworkService::worldCallback(TbotsProto::World input)
{
    ...
    // If the world is very out of date, then this is likely due to an AI reset. Clear the
    // queue
    if (!recent_world_seq_nums.empty() &&
        seq_num + RECENT_PACKET_LOSS_PERIOD <= recent_world_seq_nums.back())
    {
