...
#include "external/trinamic/tmc/ic/TMC6100/TMC6100.h"
    ...
    static const uint32_t TOTAL_NUMBER_OF_MOTORS        = 1;
    ...
    const TbotsProto::DirectControlPrimitive& direct_control)
{
    ...
    tmc4671_setTargetVelocity(0, 0);
    sleep(2);
    tmc4671_setTargetVelocity(0, 3000);
    sleep(2);
    tmc4671_setTargetVelocity(0, 0);
    sleep(2);
    tmc4671_setTargetVelocity(0, -3000);
...
void MotorService::start()
{
    ...
    // TMC4671 Setup
    //
    // Read the chip id as a sanity check for SPI communication working
    for (uint8_t motor = 0; motor < TOTAL_NUMBER_OF_MOTORS; motor++)
    {
