...
TEST(NetworkTest, test_receive)
{
    ...
    auto network_service =
        NetworkService("ff02::c3d0:42d2:bb01%" + YOUR_INTERFACE, VISION_PORT,
                       PRIMITIVE_PORT, ROBOT_STATUS_PORT, true);
    auto sender = std::make_unique<ThreadedProtoUdpSender<TbotsProto::PrimitiveSet>>(
        "ff02::c3d0:42d2:bb01%" + YOUR_INTERFACE, PRIMITIVE_PORT, true);
