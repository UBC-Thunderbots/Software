...
{
    // If the proto seems very out of date, then this is likely due to an AI reset. Clear the queue
        ...
        seq_num + RECENT_PACKET_LOSS_PERIOD <= recent_proto_seq_nums.back())
    {
        ...
        LOG(WARNING) << "Old " << proto_type << " received. Resetting sequence number tracking.";
    }
