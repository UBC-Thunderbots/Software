...
class MotorService
{
   ...
   private:
    ...
    /**
     * Returns true if we've detected a RESET in our cached motor faults indicators or if we have a fault that disables drive.
     *
    ...
    std::optional<std::chrono::time_point<std::chrono::system_clock>> tracked_motor_fault_start_time_;
