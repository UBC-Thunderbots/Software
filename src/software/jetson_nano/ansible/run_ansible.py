...
from software.jetson_nano.broadcasts.robot_broadcast_receiver import receive_announcements
import os
import argparse
...
def ansible_runner(playbook: str, options: dict = {}):
    ...
    context.CLIARGS = ImmutableDict(tags=tags, listtags=False, listtasks=False, listhosts=False, syntax=False,
                                    connection='ssh',
                                    module_path=None, forks=num_forks, remote_user=NANO_USER, private_key_file=None,
                                    ssh_common_args=None, ssh_extra_args=None, sftp_extra_args=None,
                                    scp_extra_args=None, become=False,
                                    verbosity=True, check=False, start_at_task=None, extra_vars=vars, skip_tags=skip_tags, become_ask_pass=True)
    ...
    pbex = PlaybookExecutor(playbooks=[os.path.dirname(__file__) + '/playbooks/' + playbook], inventory=inventory,
                            variable_manager=variable_manager, loader=loader, passwords={'conn_pass': ssh_pass})
...
def main():
    ...
    ap.add_argument("--ssh_pass", "-pwd", required=False, help="Password to ssh into hosts")
    ap.add_argument("--hosts", "-ho", nargs="*", required=False,
                    help="space seperated list of hosts to run on, defaults to using robot announcements",
                    default=[])
    ap.add_argument("--tags", "-t", nargs="*", required=False, help="space seperated list of tags to run", default=[])
    ap.add_argument("--skip_tags", "-st", nargs="*", required=False, help="space seperated list of tags to skip", default=[])
    ap.add_argument("--extra_vars", "-e", nargs="*", required=False,
                    help="space seperated list of variables to set in the form key=value", default=[])
