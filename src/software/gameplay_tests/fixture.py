...
def gameplay_test_runner():
    ...
    if args.run_field_test:
        yield from create_field_test_runner(args)
    else:
        yield from create_simulated_test_runner(args)


def create_field_test_runner(args):
    # Grab the current test name to store the proto log for the test case
    current_test = get_current_pytest()
    test_name = get_current_pytest().split("-")[0]
...
def create_simulated_test_runner(args):
    # Grab the current test name to store the proto log for the test case
    current_test = get_current_pytest()
    test_name = get_current_pytest().split("-")[0]
