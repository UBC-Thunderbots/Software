...
class SimulationContactListenerTest : public testing::Test
{
   protected:
    ...
    createWorld(const Robot& robot, const Ball& ball)
    {
        ...
        physics_ball  = std::make_shared<PhysicsBall>(physics_world, ball.currentState().ballState(), 1.0, 9.8);
        physics_robot = std::make_shared<PhysicsRobot>(robot.id(), physics_world, robot.currentState().robotState(), 1.0);
