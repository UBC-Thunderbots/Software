...
void PhysicsWorld::initWorld(const World& world)
{
    ...
    physics_ball  = std::make_shared<PhysicsBall>(b2_world, world.ball().currentState().ballState(), BALL_MASS_KG,
    ...
    for (const auto& friendly_robot : world.friendlyTeam().getAllRobots())
    {
        friendly_physics_robots.emplace_back(std::make_shared<PhysicsRobot>(
                friendly_robot.id(),
            b2_world, friendly_robot.currentState().robotState(), ROBOT_WITH_BATTERY_MASS_KG));
    }
    ...
    for (const auto& enemy_robot : world.enemyTeam().getAllRobots())
    {
        enemy_physics_robots.emplace_back(std::make_shared<PhysicsRobot>(
                enemy_robot.id(),
            b2_world, enemy_robot.currentState().robotState(), ROBOT_WITH_BATTERY_MASS_KG));
    }
...
World PhysicsWorld::getWorld() const
{
    ...
    if (physics_ball)
    {
        TimestampedBallState timestamped_ball_state(physics_ball->getBallState(), current_timestamp);
    ...
    {
        TimestampedRobotState timestamped_robot_state(robot->getRobotState(), current_timestamp);
