...
MATCHER(RobotStateWithIdEq, "Robot State with Id Equal") {
    return ::Test::TestUtil::equalWithinTolerance(::std::get<0>(arg), ::std::get<1>(arg), 1e-6, Angle::fromDegrees(0.1));
}
...
TEST(PhysicsWorldTest, test_add_single_yellow_robot_with_valid_id) {
    ...
    RobotState robot_state(Point(1, 0), Vector(0, 0), Angle::quarter(), AngularVelocity::half());
    ...
    EXPECT_THAT(states, ::testing::UnorderedPointwise(RobotStateWithIdEq(), physics_world.getYellowRobotStates()));
}

TEST(PhysicsWorldTest, test_add_multiple_yellow_robot_with_valid_ids) {
    PhysicsWorld physics_world(::Test::TestUtil::createSSLDivBField());

    RobotState robot_state1(Point(1, 0), Vector(0, 0), Angle::quarter(), AngularVelocity::half());
    RobotState robot_state2(Point(0, 0), Vector(3, 0), Angle::half(), AngularVelocity::quarter());
    RobotState robot_state3(Point(-2, -2), Vector(0, 4), Angle::zero(), AngularVelocity::zero());
    std::vector<PhysicsWorld::RobotStateWithId> states = {
...
TEST(PhysicsWorldTest, test_add_single_blue_robot_with_valid_id) {
    ...
    EXPECT_THAT(states, ::testing::UnorderedPointwise(RobotStateWithIdEq(), physics_world.getBlueRobotStates()));
}
...
TEST(PhysicsWorldTest, is_yellow_robot_id_available_with_unavailable_id) {
    ...
    RobotState robot_state(Point(0, 0), Vector(0, 0), Angle::zero(), AngularVelocity::zero());
    std::vector<PhysicsWorld::RobotStateWithId> states = {
            PhysicsWorld::RobotStateWithId{.id = 0, .robot_state=robot_state},
    };
    physics_world.addYellowRobots(states);
