...
int main(int argc, char *argv[])
{
    ...
    auto network_interface = DynamicParameters->getNetworkConfig()->NetworkInterface()->value();
    int yellow_team_channel = DynamicParameters->getStandaloneSimulatorConfig()->YellowTeamChannel()->value();
    std::string yellow_team_ip = std::string(MULTICAST_CHANNELS[yellow_team_channel])+ "%" + network_interface;
    int blue_team_channel = DynamicParameters->getStandaloneSimulatorConfig()->BlueTeamChannel()->value();
    std::string blue_team_ip = std::string(MULTICAST_CHANNELS[blue_team_channel])+ "%" + network_interface;
    ...
    ThreadedProtoMulticastSender<SSL_WrapperPacket> wrapper_packet_sender(SSL_VISION_DEFAULT_MULTICAST_ADDRESS, SSL_VISION_MULTICAST_PORT);
    simulator.registerOnSSLWrapperPacketReadyCallback(
            [&standalone_simulator_gui_wrapper, &wrapper_packet_sender](SSL_WrapperPacket packet) {
                wrapper_packet_sender.sendProto(packet);
                standalone_simulator_gui_wrapper.onValueReceived(packet);
            });

    ThreadedProtoMulticastListener<PrimitiveSetMsg> yellow_team_primitive_listener(yellow_team_ip, PRIMITIVE_PORT, [&simulator](PrimitiveSetMsg msg) {
    ...
    ThreadedProtoMulticastListener<PrimitiveSetMsg> blue_team_primitive_listener(blue_team_ip, PRIMITIVE_PORT, [&simulator](PrimitiveSetMsg msg) {
        for(const auto& primitive : msg.robot_primitives()) {
            RobotId id = primitive.first;
            auto [primitive_index, primitive_params] = decodePrimitiveMsg(primitive.second);
