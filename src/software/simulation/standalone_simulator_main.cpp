...
int main(int argc, char *argv[])
{
    ...
    auto simulation_mode_callback = [&standalone_simulator](StandaloneSimulator::SimulationMode new_simulation_mode) {
        switch(new_simulation_mode) {
            ...
            case StandaloneSimulator::SimulationMode::SLOW_MOTION:
                standalone_simulator.setSlowMotionMultiplier(StandaloneSimulator::DEFAULT_SLOW_MOTION_MULTIPLIER);
                standalone_simulator.startSimulation();
                break;
        }
    };

    ThreadedStandaloneSimulatorGUI threaded_standalone_simulator_gui(ball_placement_callback, simulation_mode_callback);
