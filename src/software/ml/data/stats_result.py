...
from software.evaluation.events.tracked_event import TrackedEvent, Team, EventType
...
class StatsResult(IResult):
  ...
  _field: tbots_cpp.Field = field(init=False, repr=False) = None
  
  def _is_in_enemy_half(self, point_to_check: Point):
    if not self._field:
      self._field = tbots_cpp.Field.createSSLDivisionBField()
      
    point = tbots_cpp.Point(
      point_to_check.x_meters, point_to_check.y_meters
    )
  ...
  def update_result(self, event: IEvent) -> None:    
    tracked_event = cast(TrackedEvent, event)
    
    for_friendly = tracked_event.for_team == self.friendly_team
    
    self.ball_in_enemy_half = self._is_in_enemy_half(tracked_event.world_state.ball_state.global_position)
