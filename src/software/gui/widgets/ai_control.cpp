...
{
    auto start_ai_func = []() {
        Util::DynamicParameters->getMutableAIConfig()->mutableRunAI()->setValue(true);
    };
    QWidget::connect(widget->start_ai_button, &QPushButton::clicked, start_ai_func);
    auto stop_ai_func = []() {
        Util::DynamicParameters->getMutableAIConfig()->mutableRunAI()->setValue(false);
    };
...
void setupTeamColourComboBox(Ui::AutoGeneratedMainWidget *widget)
{
    ...
    auto on_team_colour_changed = [](const QString &text) {
        if (text == "Yellow")
        {
            Util::DynamicParameters->getMutableAIConfig()
