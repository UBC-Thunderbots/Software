...
{
    auto start_ai_func = []() { Util::DynamicParameters->getMutableAIConfig()->mutableRunAI()->setValue(true); };
    QWidget::connect(widget->start_ai_button, &QPushButton::clicked, start_ai_func);
    auto stop_ai_func = []() { Util::DynamicParameters->getMutableAIConfig()->mutableRunAI()->setValue(false); };
...
void setupTeamColourComboBox(Ui::AutoGeneratedMainWidget *widget)
{
    ...
    auto on_team_colour_changed = [](const QString &text) {
        if (text == "Yellow")
        {
            Util::DynamicParameters->getMutableAIConfig()->getMutableRefboxConfig()->mutableOverrideRefboxFriendlyTeamColor()
                ->setValue(true);
            Util::DynamicParameters->getMutableAIConfig()->getMutableRefboxConfig()->mutableFriendlyColorYellow()->setValue(true);
        }
        else if (text == "Blue")
        {
            Util::DynamicParameters->getMutableAIConfig()->getMutableRefboxConfig()->mutableOverrideRefboxFriendlyTeamColor()
                ->setValue(true);
            Util::DynamicParameters->getMutableAIConfig()->getMutableRefboxConfig()->mutableFriendlyColorYellow()->setValue(false);
        }
...
void setupGameStateOverrideComboBox(Ui::AutoGeneratedMainWidget *widget)
{
    ...
    auto on_gamestate_changed = [](const QString &text) {
        if (text == "Use Refbox")
        {
            Util::DynamicParameters->getAIConfig()->getMutableRefboxConfig()->mutableOverrideRefboxPlay()->setValue(false);
        }
        else
        {
            Util::DynamicParameters->getMutableAIConfig()->getMutableRefboxConfig()->mutableOverrideRefboxPlay()->setValue(true);
            Util::DynamicParameters->getMutableAIConfig()->getMutableRefboxConfig()->mutableCurrentRefboxPlay()->setValue(
                text.toStdString());
        }
    };
...
void setupDefendingSideComboBox(Ui::AutoGeneratedMainWidget *widget)
{
    ...
    auto on_defending_side_changed = [](const QString &text) {
        if (text == "Use Refbox")
        {
            Util::DynamicParameters->getMutableAIConfig()->getMutableRefboxConfig()->mutableOverrideRefboxDefendingSide()->setValue(
                false);
        }
        else if (text == "East")
        {
            ...
            Util::DynamicParameters->getMutableAIConfig()->getMutableRefboxConfig()->mutableDefendingPositiveSide()->setValue(false);
        }
        else if (text == "West")
        {
            Util::DynamicParameters->getMutableAIConfig()->mutableOverrideAIPlay()->setValue(false);
            // TODO: Confirm how East and West map to positive and negative sides
            Util::DynamicParameters->getMutableAIConfig()->getMutableRefboxConfig()->mutableDefendingPositiveSide()->setValue(true);
        }
        else
        {
...
void setupPlayOverrideComboBox(Ui::AutoGeneratedMainWidget *widget)
{
    ...
    auto on_play_changed = [](const QString &text) {
        ...
        {
            Util::DynamicParameters->getMutableAIConfig()->mutableOverrideAIPlay()->setValue(false);
            Util::DynamicParameters->getMutableAIConfig()->mutableCurrentAIPlay()->setValue(text.toStdString());
        }
    };
