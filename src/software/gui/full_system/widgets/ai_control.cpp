...
// TODO TARA: fix QList formatting, use a different type of list? and then re-build ai_control. Then add a comment to sensor_fusion.cpp about if goalie is overrideable or not
        ...
        std::shared_ptr<Parameter<int>> enemy_goalie_id_parameter)
{
    ...
    auto on_enemy_goalie_id_changed =
            [override_enemy_goalie_id_parameter,
                    enemy_goalie_id_parameter](const QString &text) {
                ...
                if (override)
                {
                    override_enemy_goalie_id_parameter->setValue(true);
                    enemy_goalie_id_parameter->setValue(goalie_id);
                }
                else if (text == "Use GameController")
                {
                    override_enemy_goalie_id_parameter->setValue(false);
                }
                else
                {
                    LOG(FATAL) << "Tried to set the friendly goalie ID with an invalid value: '"
