...
{
    setupAIStartAndStopButtons(widget, config->getMutableAIControlConfig()->mutableRunAI());
    setupTeamColourComboBox(widget, config->getMutableSensorFusionConfig()->mutableOverrideRefboxFriendlyTeamColor(), config->getMutableSensorFusionConfig()->mutableFriendlyColorYellow());
    setupDefendingSideComboBox(widget, config->getMutableSensorFusionConfig()->mutableOverrideRefboxDefendingSide(), config->getMutableSensorFusionConfig()->mutableDefendingPositiveSide());
    setupGameStateOverrideComboBox(widget, config->getMutableAIControlConfig()->mutableOverrideRefboxGameState(), config->getMutableAIControlConfig()->mutablePreviousRefboxGameState(), config->getMutableAIControlConfig()->mutableCurrentRefboxGameState());
    setupPlayOverrideComboBox(widget, config->getMutableAIControlConfig()->mutableOverrideAIPlay(), config->getMutableAIControlConfig()->mutableCurrentAIPlay());
    setupChannelSpinBox(widget, config->getMutableNetworkConfig()->mutableChannel());
    setupFriendlyGoalieIDSpinBox(widget, config->getMutableSensorFusionConfig()->mutableFriendlyGoalieId());
    setupEnemyGoalieIDSpinBox(widget, config->getMutableSensorFusionConfig()->mutableEnemyGoalieId());
}
...
{
    auto start_ai_func = [run_ai_parameter]() {
        run_ai_parameter->setValue(true);
    };
    QWidget::connect(widget->start_ai_button, &QPushButton::clicked, start_ai_func);
    auto stop_ai_func = [run_ai_parameter]() {
        run_ai_parameter->setValue(false);
    };
...
void setupTeamColourComboBox(Ui::AutogeneratedFullSystemMainWidget *widget,
                             std::shared_ptr<Parameter<bool>> override_friendly_colour_parameter, std::shared_ptr<Parameter<bool>> friendly_colour_yellow_parameter)
{
    ...
    auto on_team_colour_changed = [override_friendly_colour_parameter, friendly_colour_yellow_parameter](const QString &text) {
        if (text == "Yellow")
        {
...
void setupDefendingSideComboBox(Ui::AutogeneratedFullSystemMainWidget *widget,
                                std::shared_ptr<Parameter<bool>> defending_side_override_parameter, std::shared_ptr<Parameter<bool>> defending_positive_side_parameter)
{
    ...
    auto on_defending_side_changed = [defending_side_override_parameter, defending_positive_side_parameter](const QString &text) {
        if (text == "Use Refbox")
        {
...
void setupGameStateOverrideComboBox(Ui::AutogeneratedFullSystemMainWidget *widget,
                                    std::shared_ptr<Parameter<bool>> gamestate_override_parameter, std::shared_ptr<Parameter<std::string>> previous_gamestate_parameter, std::shared_ptr<Parameter<std::string>> current_gamestate_parameter)
{
    ...
    auto on_gamestate_changed = [gamestate_override_parameter, previous_gamestate_parameter, current_gamestate_parameter](const QString &text) {
        if (text == "Use Refbox")
        {
        ...
        else
        {
            ...
            previous_gamestate_parameter->setValue(current_gamestate_parameter->value());
            current_gamestate_parameter->setValue(text.toStdString());
        }
    };
...
void setupPlayOverrideComboBox(Ui::AutogeneratedFullSystemMainWidget *widget,
                               std::shared_ptr<Parameter<bool>> play_override_parameter, std::shared_ptr<Parameter<std::string>> current_play_parameter)
{
    ...
    auto on_play_changed = [play_override_parameter, current_play_parameter](const QString &text) {
        ...
        else
        {
            ...
            current_play_parameter->setValue(
                text.toStdString());
        }
    };
