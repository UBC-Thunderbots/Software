                             ...
                             std::shared_ptr<ThunderbotsConfig> config)
{
    ...
    auto on_team_colour_changed = [config](const QString &text) {
        auto sensor_fusion_config =
            config->getMutableSensorFusionConfig();
        if (text == "Yellow")
        {
            sensor_fusion_config->mutableOverrideRefboxFriendlyTeamColor()->setValue(true);
        ...
        {
            sensor_fusion_config->mutableOverrideRefboxFriendlyTeamColor()->setValue(false);
        }
        else
        {
                                    ...
                                    std::shared_ptr<ThunderbotsConfig> config)
{
    ...
    auto on_gamestate_changed = [config](const QString &text) {
        auto ai_control_config =
            config->getMutableAIControlConfig();
        if (text == "Use Refbox")
        {
            ai_control_config->mutableOverrideRefboxPlay()->setValue(false);
        }
        else
        {
            ai_control_config->mutableOverrideRefboxPlay()->setValue(true);
            ai_control_config->mutableCurrentRefboxPlay()->setValue(text.toStdString());
        }
    };
