...
void setupSensorStatus(Ui::AutoGeneratedMainWidget* widget)
{
    ...
    widget->progressBar_dribbler_speed->setupProgressBar(6000, "%1 RPM");
                        ...
                        const SensorProto& sensor_msg)
{
    for (const auto& robot_msg : sensor_msg.robot_status_msgs())
    {
        ...
        widget->progressBar_battery->updateProgressBar(robot_msg.power_status().battery_voltage());
        widget->progressBar_capacitor->updateProgressBar(robot_msg.power_status().capacitor_voltage());
        widget->progressBar_dribbler_speed->updateProgressBar(robot_msg.dribbler_status().dribbler_rpm());

        // break beam reading is from [0,1] and we want it displayed as a percent so it is
        // multiplied by 100 to map to [0,100]
        widget->progressBar_break_beam->updateProgressBar(robot_msg.break_beam_status().break_beam_reading() * 100);

        // update temperature progress bars
        widget->progressBar_dribbler_temp->updateProgressBar(robot_msg.temperature_status().dribbler_temperature());
        widget->progressBar_board_temp->updateProgressBar(robot_msg.temperature_status().board_temperature());
    }
}
