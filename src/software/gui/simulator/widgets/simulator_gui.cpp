...
SimulatorGUI::SimulatorGUI(std::shared_ptr<ThreadSafeBuffer<SSL_WrapperPacket>> ssl_wrapper_packet_buffer, std::shared_ptr<ThreadSafeBuffer<Rectangle>> view_area_buffer) : QMainWindow(), main_widget(new MainWidget(this)),
                                                                                                             update_timer(new QTimer(this)), ssl_wrapper_packet_buffer(ssl_wrapper_packet_buffer), view_area_buffer(view_area_buffer) {
    setCentralWidget(main_widget);
    connect(update_timer, &QTimer::timeout, this, &SimulatorGUI::drawSimulatorContents);
    update_timer->start(static_cast<int>(Duration::fromSeconds(DRAW_UPDATE_INTERVAL_SECONDS).getMilliseconds()));
}
