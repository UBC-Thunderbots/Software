      ...
      config(config)
{
    ...
    connect(main_widget->play_button, &QRadioButton::toggled, [&config](bool checked) {
        if(checked) {
            config->mutableSimulationSpeedMode()->setValue(0);
            std::cout << "Set simulation to Play" << std::endl;
        }
    });
    connect(main_widget->pause_button, &QRadioButton::toggled, [&config](bool checked) {
        if(checked) {
            config->mutableSimulationSpeedMode()->setValue(1);
            std::cout << "Set simulation to Pause" << std::endl;
        }
    });
    connect(main_widget->slow_motion_button, &QRadioButton::toggled, [&config](bool checked) {
        if(checked) {
            config->mutableSimulationSpeedMode()->setValue(2);
            std::cout << "Set simulation to Slow Motion" << std::endl;
        }
    });
