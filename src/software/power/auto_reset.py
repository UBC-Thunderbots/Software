...
def set_gpio(pin, value):
    try:
        ...
        try:
            subprocess.run(["pinctrl", "set", str(pin), "op", v], check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        except Exception:
            subprocess.run(["raspi-gpio", "set", str(pin), "op", v], check=False, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)

def before_upload(source, target, env):
    print(f"Setting ESP32 to bootloader mode using GPIO {BOOT_GPIO} (BOOT) and GPIO {RESET_GPIO} (EN)...")
