...
class RefereeTracker(Tracker):
  def __init__(self, friendly_color_yellow: bool, callback: Callable[[int, int, int], None], buffer_size: int = 5):
  ...
  def refresh(self):
    ...
    if refree_msg.HasField("yellow" if self.friendly_colour_yellow else "blue"):
      team_info = refree_msg.yellow if self.friendly_colour_yellow else refree_msg.blue
