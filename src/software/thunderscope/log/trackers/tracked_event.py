...
def get_event_from_world(world: tbots_cpp.World, event_type: EventType) -> TrackedEvent:
  """
  ...
  return TrackedEvent(
    timestamp=world_msg.time_sent.ToDatetime().timestamp(),
...
def event_to_csv_row(event: TrackedEvent) -> str:
  ...
  for r in event.friendly_robot_states:
      row.extend([r.position[0], r.position[1], r.orientation, r.velocity[0], r.velocity[1], r.angular_velocity])
