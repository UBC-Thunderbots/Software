...
class ControllerDiagnostics(object):
    ...
    def process_event(self, event) :
        if (event.type == ecodes.EV_ABS) :
            ...
            print("EVENT: " + event_t)
            if (event_t == "ABS_X"):
                self.update_move(x=None, y=absevent.event.value / XBOX_MAX_RANGE * MAX_LINEAR_SPEED_MPS)
            elif (event_t == "ABS_Y"):
                self.update_move(x=-absevent.event.value / XBOX_MAX_RANGE * MAX_LINEAR_SPEED_MPS, y=None)
            elif (event_t == "ABS_RX"):
                self.update_angular_velocity(absevent.event.value / XBOX_MAX_RANGE *MAX_ANGULAR_SPEED_RAD_PER_S)
            elif(event_t == "ABS_HAT0Y"):
                self.update_dribbler(self.dribbler_speed - absevent.event.value * DRIBBLER_STEPPER)
            ...
            if (event_t == "ABS_X" or event_t == "ABS_Y" or event_t == "ABS_RX" or event_t == "ABS_HAT0Y" or event_t == "ABS_RZ") :
    ...
    def kick(self) :
        ...
        power_control.chicker.auto_chip_or_kick.autokick_speed_m_per_s = ( self.power )
    ...
    def update_dribbler(self, new_dribbler_speed):
        if (new_dribbler_speed < MAX_DRIBBLER_RPM and new_dribbler_speed > MIN_DRIBBLER_RPM) :
    ...
    def update_angular_velocity(self, ang_vel) :
        ...
        self.move_x = 0
        self.move_y = 0
        print("new ang vel: " + str(self.ang_vel))
