...
class GlFSStatsLayer(GLLayer):
  def __init__(self, name: str, friendly_colour_yellow: bool, buffer_size: int = 5, record_enemy_stats: bool = False):
    super().__init__(name)
  ...
  def refresh_graphics(self) -> None:
      ...
      if attacker_vis_msg.HasField("shot"):
        if attacker_vis_msg.shot.HasField("shot_origin"):
          ...
          shot_origin_point = tbots_cpp.Point(shot_origin.x_meters, shot_origin.y_meters)

          if self.latest_shot_on_net is None or self.latest_shot_on_net != shot_origin_point:
      ...
      if self.record_enemy_stats and refree_msg.HasField("blue" if self.friendly_colour_yellow else "yellow"):
        self._record_referee_stats(
  ...
  def _write_stats_to_file(self, stats: FSStats, file_name: str) -> None:
    file_path = os.path.join(RuntimeManagerConstants.RUNTIME_STATS_DIRECTORY_PATH, file_name)
    ...
    try:
      ...
      logging.warning(
        f"Failed to write TOML FS stats file at: {file_path}"
      )
