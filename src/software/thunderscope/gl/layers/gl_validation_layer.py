...
class GLValidationOverlayLayer(GLLayer):
    ...
    def draw_overlay(
        ...
        for validation in self.validation_layer.get_validations():
            if validation.status == protos.ValidationStatus.FAILING:
...
class GLValidationLayer(GLLayer):
    ...
    def __init__(
        ...
        self.validation_set_buffer = ThreadSafeBuffer(buffer_size, protos.ValidationProtoSet)
        self.cached_always_validation_set = protos.ValidationProtoSet()
        self.cached_eventually_validation_set = protos.ValidationProtoSet()
    ...
    def refresh_graphics(self) -> None:
        ...
        for _ in range(self.validation_set_buffer.size()):
            ...
            if self.validation_set.validation_type == protos.ValidationType.ALWAYS:
        ...
        for validation in self.cached_eventually_validation_set.validations:
            if validation.status == protos.ValidationStatus.PASSING:
    ...
    def get_validations(self) -> list[protos.ValidationProto]:
    ...
    def __update_validation_graphics(self, validations: list[protos.ValidationProto]) -> None:
    ...
    def __get_validation_color(
        self, validation_status: protos.ValidationStatus
    ) -> QtGui.QColor:
        ...
        return (
            ...
            if validation_status == protos.ValidationStatus.PASSING
