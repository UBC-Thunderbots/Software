...
TEST(ThreadedEstopReaderTest, estop_tick_is_called_multiple_times)
{
    ...
    EXPECT_CALL(*mock_uart_ptr, serialRead(_))
            ...
            .WillOnce(Return(play_ret_val))
            .WillOnce(Return(play_ret_val))
            .WillOnce(DoAll( InvokeWithoutArgs([&cv, &ready] { ready = true; cv.notify_one(); }), Return(play_ret_val)))
            .WillRepeatedly(Return(play_ret_val));

    ThreadedEstopReader estopReader(std::move(mock_uart), startup_interval_ms);
    auto now = std::chrono::system_clock::now();
    EXPECT_TRUE(cv.wait_until(lock, now + std::chrono::milliseconds(test_timeout_ms), [&ready]{return ready;}));
}
...
TEST(ThreadedEstopReaderTest, estop_state_changes_based_on_read_val)
{
    ...
    EXPECT_TRUE(cv.wait_until(lock, std::chrono::system_clock::now() + std::chrono::milliseconds(startup_interval_ms + test_timeout_ms), [&ready]{return ready == true;}));
    ...
    EXPECT_CALL(*mock_uart_ptr, serialRead(_))
            ...
            .WillOnce(DoAll( InvokeWithoutArgs([&cv, &ready] { ready = true; cv.notify_one();}), Return(stop_ret_val)))
            .WillRepeatedly(Return(stop_ret_val));
    EXPECT_TRUE(cv.wait_until(lock, std::chrono::system_clock::now() + std::chrono::milliseconds(test_timeout_ms), [&ready]{return ready == true;}));
    EXPECT_FALSE(estopReader.isEstopPlay());
...
TEST(ThreadedEstopReaderTest, estop_play_is_false_after_reading_unexpected_message)
{
    ...
    EXPECT_CALL(*mock_uart_ptr, serialRead(_))
            ...
            .WillOnce(DoAll( InvokeWithoutArgs([&cv, &ready] { ready = true; cv.notify_one();}), Return(garbage_ret_val)))
            .WillOnce(Return(garbage_ret_val))
            .WillOnce(Return(garbage_ret_val))
            .WillRepeatedly(Return(play_ret_val));
     ...
     estop_play_is_true_after_reading_play_message_following_multiple_unexpected_message)
{
    ...
    EXPECT_CALL(*mock_uart_ptr, serialRead(_))
            ...
            .WillOnce(Return(garbage_ret_val))
            .WillOnce(Return(play_ret_val))
            .WillRepeatedly(DoAll(InvokeWithoutArgs([&cv, &ready] { ready = true; cv.notify_one();}), Return(play_ret_val)));
