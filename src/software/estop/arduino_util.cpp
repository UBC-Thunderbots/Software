...
std::optional<std::string> ArduinoUtil::getArduinoPort(){
    ...
    for(auto device : devices){
        ...
        if(hwInfo.has_value()){
            if (hwInfo.value().vendor == ARDUINO_VENDOR_ID && hwInfo.value().product == ARDUINO_PRODUCT_ID){
...
std::optional<ArduinoUtil::HwInfo> ArduinoUtil::getInfo(std::string port){
    //ported from https://github.com/pyserial/pyserial/blob/bce419352b22b2605df6c2158f3e20a15b8061cb/serial/tools/list_ports_linux.py

    std::string device_path_string = (boost::format("/sys/class/tty/%1%/device") % port).str();
    ...
    subsystem = boost::filesystem::basename(realpath((device_path / "subsystem").c_str(), NULL));
...
std::vector<std::string> ArduinoUtil::getSerialDevices(){
    ...
    for(boost::filesystem::directory_iterator itr(dev) ; itr != end_itr; ++itr){
        ...
        if(filename.find("ttyUSB") != std::string::npos || filename.find("ttyACM") != std::string::npos || filename.find("ttyS") != std::string::npos){
