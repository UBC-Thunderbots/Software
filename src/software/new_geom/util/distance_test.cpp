...
TEST(DistanceTest, point_near_polygon_closest_to_edge)
{
    ...
    Point point(0.5, 1.1);
    double expected = 0.1;
    EXPECT_NEAR(distance(point, polygon), expected, GeomConstants::FIXED_EPSILON_ONE);
    EXPECT_NEAR(distance(polygon, point), expected, GeomConstants::FIXED_EPSILON_ONE);
}
