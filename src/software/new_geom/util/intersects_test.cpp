...
TEST(IntersectsTest, ray_inside_polygon)
{
    ...
    EXPECT_TRUE(intersectsNew(p, r));
    EXPECT_TRUE(intersectsNew(r, p));
}
...
TEST(IntersectsTest, ray_outside_polygon_not_intersecting)
{
    ...
    EXPECT_FALSE(intersectsNew(p, r));
    EXPECT_FALSE(intersectsNew(r, p));
}
...
TEST(IntersectsTest, circle_tangent_to_triangle_edge)
{
    ...
    Circle c({0, -1}, 1);
    EXPECT_FALSE(intersectsNew(t, c));
    EXPECT_FALSE(intersectsNew(c, t));
}
...
TEST(IntersectsTest, circle_completely_inside_triangle)
{
    ...
    Circle c({0, 5}, 1);
    EXPECT_TRUE(intersectsNew(t, c));
    EXPECT_TRUE(intersectsNew(c, t));
}
...
TEST(IntersectsTest, circle_inside_rectangle)
{
    Rectangle r({-5, -2}, {3, 4});
    Circle c({0, 0}, 1);
    EXPECT_TRUE(intersectsNew(r, c));
    EXPECT_TRUE(intersectsNew(c, r));
}
...
TEST(IntersectsTest, circle_outside_rectangle_not_intersecting)
{
    Rectangle r({-5, -2}, {3, 4});
    Circle c({4, -5}, 2);
    EXPECT_FALSE(intersectsNew(r, c));
    EXPECT_FALSE(intersectsNew(c, r));
}
...
TEST(IntersectsTest, circles_same_origin)
{
    Circle c1({4, -5}, 3);
    Circle c2({4, -5}, 2);
    EXPECT_TRUE(intersectsNew(c1, c2));
}
...
TEST(IntersectsTest, segment_completely_inside_circle)
{
    Segment s({-8, 4}, {-4, 0});
    Circle c({-6, 2}, 4);
    EXPECT_FALSE(intersectsNew(s, c));
    EXPECT_FALSE(intersectsNew(c, s));
}
...
TEST(IntersectsTest, segment_circle_intersecting)
{
    Segment s({-8, 2}, {-10, 6});
    Circle c({-6, 2}, 4);
    EXPECT_TRUE(intersectsNew(s, c));
    EXPECT_TRUE(intersectsNew(c, s));
}
...
TEST(IntersectsTest, segments_overlapping)
{
    ...
    EXPECT_TRUE(intersectsNew(s1, s2));
    EXPECT_TRUE(intersectsNew(s1, s2));
}
...
TEST(IntersectsTest, segments_end_points_overlapping)
{
    ...
    EXPECT_TRUE(intersectsNew(s1, s2));
    EXPECT_TRUE(intersectsNew(s2, s1));
}
...
TEST(IntersectsTest, segments_far_from_each_other)
{
    ...
    EXPECT_FALSE(intersectsNew(s1, s2));
    EXPECT_FALSE(intersectsNew(s2, s1));
}
...
TEST(IntersectsTest, test_seg_crosses_seg)
{
    ...
    for (int i = 0; i < 10; i++)
    {
        ...
        bool found    = intersectsNew(Segment(a1, a2), Segment(b1, b2));
...
TEST(IntersectsTest, test_vector_crosses_seg)
{
    ...
    for (int i = 0; i < 5; i++)
    {
        ...
        bool found = intersectsNew(Ray(a1, ray_direction), Segment(b1, b2));
     ...
     test_ray_intersect_position_and_direction_intersect_not_just_direction)
{
    ...
    EXPECT_EQ(intersectsNew(position_and_direction, segment), false);
...
TEST(IntersectsTest, ray_parallel_to_segment_not_intersecting)
{
    ...
    Segment s({5, -5}, {8, 4});
    EXPECT_FALSE(intersectsNew(r, s));
    EXPECT_FALSE(intersectsNew(s, r));
}
