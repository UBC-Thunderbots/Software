...
TEST(LinearSpline2dTest, test_constructor_three_knot)
{
    ...
    EXPECT_TRUE(equalWithinTolerance(s.getValueAt(0.1), Point(3.8, 14.4), 1e-9));
    EXPECT_TRUE(equalWithinTolerance(s.getValueAt(0.5), points[1], 1e-9));
    EXPECT_TRUE(equalWithinTolerance(s.getValueAt(0.6), Point(1, -0.2), 1e-8));
    EXPECT_TRUE(equalWithinTolerance(s.getValueAt(0.9), Point(-5, -0.8), 1e-9));
...
TEST(LinearSpline2dTest, test_constructor_two_knot)
{
    ...
    EXPECT_EQ(s.getNumKnots(), points.size());
    EXPECT_EQ(s.getValueAt(0.0), points[0]);
    EXPECT_TRUE(equalWithinTolerance(s.getValueAt(0.1), Point(-2.3, -15.7), 1e-9));
    EXPECT_TRUE(equalWithinTolerance(s.getValueAt(0.5), Point(0.5, 5.5), 1e-9));
    EXPECT_TRUE(equalWithinTolerance(s.getValueAt(0.6), Point(1.2, 10.8), 1e-7));
    EXPECT_TRUE(equalWithinTolerance(s.getValueAt(0.9), Point(3.3, 26.7), 1e-7));
...
TEST(LinearSpline2dTest, test_one_knot_value_at_out_of_range)
{
    ...
    EXPECT_TRUE(equalWithinTolerance(s.getValueAt(-0.1), Point(1, -18), 1e-9));
    EXPECT_TRUE(equalWithinTolerance(s.getValueAt(1.1), Point(1, -18), 1e-9));
}
...
TEST(LinearSpline2dTest, test_two_knot_value_at_out_of_range)
{
    ...
    EXPECT_TRUE(equalWithinTolerance(s.getValueAt(-0.1), Point(1, -2), 1e-9));
    EXPECT_TRUE(equalWithinTolerance(s.getValueAt(1.1), Point(1, -18), 1e-9));
}
...
TEST(LinearSpline2dTest, test_three_knot_value_at_out_of_range)
{
    ...
    EXPECT_TRUE(equalWithinTolerance(s.getValueAt(-0.1), Point(4, 18), 1e-9));
    EXPECT_TRUE(equalWithinTolerance(s.getValueAt(1.1), Point(-7, -1), 1e-9));
}
