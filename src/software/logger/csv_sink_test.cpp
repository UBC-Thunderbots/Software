...
TEST(CSVSinkTest, test_csv_log_appends)
{
    ...
    auto csv_sink_handle =
        logWorker->addSink(std::make_unique<CSVSink>(logging_dir), &CSVSink::appendToFile);
    ...
    std::string output((std::istreambuf_iterator<char>(read_test)), std::istreambuf_iterator<char>());
...
INSTANTIATE_TEST_CASE_P(
    All, CSVSinkTest,
    ::testing::Values(
        std::make_tuple<LEVELS>(LEVELS(INFO)),
