...
{
    // Test where we cannot get to the ball before it leaves the field unless we intercept
    ...
    double timetoedge_ball =
        distance(edge_of_field, ball.position()) / ball.velocity().length();

    // we find the position where the robot can't reach the ball unless it goes at max
    // speed the entire time
    double robot_distance_away = timetoedge_ball * robot_max_speed;

    // decrease this distance so that the position is reachable in fallback interception
    // only
    double reachable_distance_away = robot_distance_away - 1.3;

    Robot robot(0, {4.5, edge_of_field.y() - reachable_distance_away}, {0, 0},
    ...
    // expect the final speed to be somewhere around the final speed of the robot
    // accelerating at max value to the edge
    double expected_final_speed = robot_max_acceleration * timetoedge_ball;
    ASSERT_TRUE(std::abs(expected_final_speed -
                         best_intercept_fallback.value().final_speed) < 0.5);
}
