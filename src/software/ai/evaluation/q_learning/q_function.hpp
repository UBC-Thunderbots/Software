 ...
 * 
 * We use the Q-values returned by the Q-function to select the best action `A_t` to perform
 * in a given state `S_t` at time step `t` (this is the action selection strategy, or "policy" 
 * that the agent follows).
 * 
 * After selecting action `A_t`, we observe a reward `R_t+1` and enter a new state `S_t+1`.
 * `R_t+1` and `S_t+1` are used to update the Q-function and adjust the Q-value given 
 * for taking action `A_t` in state `S_t`. 
 * 
 * The Q function is used to learn an optimal policy, which is a policy that maximizes the 
 * expected cumulative reward over any and all successive steps, starting from the current state.
 *  
 * Resources:
