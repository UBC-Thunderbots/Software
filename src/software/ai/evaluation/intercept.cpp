...
std::optional<InterceptionResult> findBestInterceptForBall(
    ...
    bool include_fallback_interceptions)
{
    ...
    if (ball.velocity().length() < BALL_MOVING_SLOW_SPEED_THRESHOLD)
    {
        ...
        return std::make_optional<InterceptionResult>(
            {intercept_position, robot_time_to_pos, 0.0});
    }

    intercept_position += ball.velocity().normalize(INTERCEPT_POSITION_SEARCH_INTERVAL);

    // todo find how quick robot can be moving for it to trap the ball in dribbler when
    // intercepting
    ...
    while (contains(field.fieldLines(), intercept_position))
    {
        ...
        if (final_speed_to_reach_in_time < MAX_INTERCEPT_SPEED &&
            final_speed_to_reach_in_time < fallback_interception_final_speed &&
    ...
    //     if we can't reach the ball in time and we have valid fallback interception
    //     point, use it
    if (!contains(field.fieldLines(), intercept_position))
    {
            ...
            fallback_interception_point != ball.position())
        {
            ...
            robot_time_to_pos        = robot.getTimeToPosition(
                fallback_interception_point,
                (ball.position() - robot.position())
                    .normalize(fallback_interception_final_speed));
        }
        else
        {
            return {};
    ...
    return std::make_optional<InterceptionResult>(
        {intercept_position, robot_time_to_pos, interception_final_speed});
}
