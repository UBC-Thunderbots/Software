...
class DQN
{
   public:
    ...
    std::array<float, reflective_enum::size<TAction>()> act(TState state);

    void update(std::vector<Transition<TState, TAction>> batch);
...
template <typename TState, typename TAction>
std::array<float, reflective_enum::size<TAction>()> DQN<TState, TAction>::act(
    TState state)
{
    torch::NoGradGuard no_grad;
    torch::Tensor q_values = current_net_->forward(state.toTensor());

    std::array<float, reflective_enum::size<TAction>()> q_values_array;
    std::copy(q_values.data_ptr<float>(), q_values.data_ptr<float>() + q_values.numel(),
              q_values_array.begin());
    return q_values_array;
}

template <typename TState, typename TAction>
void DQN<TState, TAction>::update(std::vector<Transition<TState, TAction>> batch)
{
    torch::Tensor state_batch      = torch::empty({batch.size(), TState::size()});
    torch::Tensor action_batch     = torch::empty({batch.size()});
    torch::Tensor reward_batch     = torch::empty({batch.size()});
    torch::Tensor next_state_batch = torch::empty({batch.size(), TState::size()});

    for (size_t i = 0; i < batch.size(); ++i)
    {
        const Transition<TState, TAction>& transition = batch.at(i);

        state_batch[i]      = transition.state.toTensor();
        action_batch[i]     = transition.action;
        reward_batch[i]     = transition.reward;
        next_state_batch[i] = transition.next_state.toTensor();
    }
