                         ...
                         const Angle& chick_direction)
{
    ...
    Segment behind_ball_segment(
        ball_position + behind_ball_direction.normalize(DIST_TO_FRONT_OF_ROBOT_METERS),
        ball_position + behind_ball_direction.normalize(2.5 * ROBOT_MAX_RADIUS_METERS));

    // The width of the kicker/chipper that the ball can be within to not hit the
    // dribbler's side panels
    double chicker_width =
        robot.robotConstants().dribbler_width_meters - 2 * BALL_MAX_RADIUS_METERS;
