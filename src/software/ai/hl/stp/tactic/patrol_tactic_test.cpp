...
protected:
    ...
    void GenerateExpectedActions(Robot robot, std::vector<Point> patrolPoints, Angle angle, double speedAtPatrolPoints) {
        for(int i = 0; i < patrolPoints.size(); i++) {
            ...
            expected_action->updateControlParams(robot, patrolPoints[i], angle, speedAtPatrolPoints,
    ...
    void GenerateRobotStatesToCompleteAction(std::vector<Point> patrolPoints, Vector velocity ) {
        for (int i = 0; i < patrolPoints.size(); i++ ) {
            RobotState robotState = RobotState(patrolPoints[i], velocity, Angle::zero(),
                                               AngularVelocity::zero(), Timestamp::fromSeconds(std::time(nullptr)));
    ...
    TEST_F(PatrolTacticTest, patrol_one_point) {
        ...
        tactic.updateRobot(robot);
        GenerateExpectedActions(robot, std::vector<Point>({patrolPoint1}), Angle::zero(), speedAtPatrolPoints);
    ...
    TEST_F(PatrolTacticTest, patrol_three_points) {
        ...
        for (int i = 0; i < patrolPoints.size(); i++) {
            ...
            simulateActionToCompletion(robot, robotStateToCompleteActions[i], action_ptr, tactic);
        }
    ...
    TEST_F(PatrolTacticTest, cost_of_non_assigned_robot_when_tactic_is_assigned) {
        ...
        Robot assigned_robot = Robot(0, robotStartingPoint, initialVelocity, Angle::zero(), AngularVelocity::zero(),
                            Timestamp::fromSeconds(0));

        Robot non_assigned_robot = Robot(1, robotStartingPoint, initialVelocity, Angle::zero(), AngularVelocity::zero(),
                                 Timestamp::fromSeconds(0));
    ...
    TEST_F(PatrolTacticTest, cost_of_already_assigned_robot) {
        ...
        Robot assigned_robot = Robot(robotId, robotStartingPoint, initialVelocity, Angle::zero(), AngularVelocity::zero(),
                            Timestamp::fromSeconds(0));
