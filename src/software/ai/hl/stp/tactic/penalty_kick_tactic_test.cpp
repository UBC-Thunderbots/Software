...
TEST(PenaltyKickTacticTest, no_enemy_goalie)
{
    ...
    world = ::TestUtil::setBallVelocity(world, Vector(0, 0),
					Timestamp::fromSeconds(0));

    Vector behind_ball_direction =
      -(world.ball().position() - world.field().enemyGoalCenter()).normalize();
    Point behind_ball = world.field().penaltyEnemy() + behind_ball_direction.normalize(
                                                        DIST_TO_FRONT_OF_ROBOT_METERS +
                                                        BALL_MAX_RADIUS_METERS);

    Robot shooter = Robot(0, behind_ball, Vector(0, 0),
		      behind_ball_direction.orientation(), AngularVelocity::zero(),
		      Timestamp::fromSeconds(0));
    PenaltyKickTactic tactic = PenaltyKickTactic(world.ball(), world.field(),
						 std::nullopt, false);
...
TEST(PenaltyKickTacticTest, enemy_goalie_offset_left_no_viable_shot)
{
    ...
    Vector behind_ball_direction = -(world.ball().position() -
    ...
    PenaltyKickTactic tactic = PenaltyKickTactic(world.ball(), world.field(),
						 std::optional<Robot>{enemy_goalie}, false);
...
TEST(PenaltyKickTacticTest, enemy_goalie_offset_right_no_viable_shot)
{
    ...
    Point enemy_goalie_pos = Point(world.field().enemyGoalCenter().x(), -0.2);
    Vector behind_ball_direction = -(world.ball().position() - enemy_goalie_pos)
		.normalize();
    Point behind_ball = world.ball().position()
      + behind_ball_direction.normalize(DIST_TO_FRONT_OF_ROBOT_METERS +
					BALL_MAX_RADIUS_METERS);
    
    Robot enemy_goalie = Robot(0, enemy_goalie_pos, Vector(0, +0.2),
...
TEST(PenaltyKickTacticTest, enemy_goalie_right_viable_shot_left)
{
    ...
    Point enemy_goalie_pos = Point(world.field().enemyGoalCenter().x(), +0.2);
    Vector behind_ball_direction = -(world.ball().position()
									 - world.field().enemyGoalpostNeg()).normalize();
    Point behind_ball = world.ball().position()
      + behind_ball_direction.normalize(DIST_TO_FRONT_OF_ROBOT_METERS +
										BALL_MAX_RADIUS_METERS);
...
TEST(PenaltyKickPlayTest, enemy_goalie_left_viable_shot_right)
{
	World world = ::TestUtil::createBlankTestingWorld();
    world = ::TestUtil::setBallPosition(world, world.field().penaltyEnemy(),
					Timestamp::fromSeconds(0));
...
TEST(PenaltyKickPlayTest, no_enemy_goalie_shot_position)
{
	World world = ::TestUtil::createBlankTestingWorld();
    world = ::TestUtil::setBallPosition(world, world.field().penaltyEnemy(),
					Timestamp::fromSeconds(0));
	...
	EXPECT_EQ(tactic.evaluateNextPosition(), world.field().enemyGoalCenter());
}
