...
TEST(GoalieFSMTest, test_get_goalie_position_to_block_2)
{
    Field field = Field::createSSLDivisionBField();
    TbotsProto::GoalieTacticConfig goalie_tactic_config;
    World world  = ::TestUtil::createBlankTestingWorld();
    Ball ball = Ball(Point(-2.5, 0), Vector(0, 0), Timestamp::fromSeconds(123));

    Point cd1 = CreaseDefenderFSM::findBlockThreatPoint(field, ball.position(),TbotsProto::CreaseDefenderAlignment::LEFT, 1.0).value();
    Point cd2 = CreaseDefenderFSM::findBlockThreatPoint(field, ball.position(),TbotsProto::CreaseDefenderAlignment::RIGHT, 1.0).value();
    world = ::TestUtil::setFriendlyRobotPositions(world, {Point(-4.5, 0), cd1, cd2},Timestamp());


    Point goalie_pos =
            GoalieFSM::getGoaliePositionToBlock(world.friendlyTeam(), ball, field, goalie_tactic_config);
    std::cout<<goalie_pos<<std::endl;
    EXPECT_TRUE(contains(field.friendlyDefenseArea(), goalie_pos));
    EXPECT_EQ(Point(field.friendlyDefenseArea().xMax(), 0), goalie_pos);
