...
void GoalieTactic::calculateNextAction(ActionCoroutine::push_type &yield)
{
    ...
    do
    {
        ...
        auto ball_speed_panic =
            Util::DynamicParameters->getAIConfig()->getGoalieTacticConfig()->BallSpeedPanic()->value();
        // what should the final goalie speed be, so that the goalie accelerates faster
        auto goalie_final_speed =
            Util::DynamicParameters->getAIConfig()->getGoalieTacticConfig()->GoalieFinalSpeed()->value();
        // how far in should the goalie wedge itself into the block cone, to block balls
        auto block_cone_radius =
            Util::DynamicParameters->getAIConfig()->getGoalieTacticConfig()->BlockConeRadius()->value();
        // by how much should the defense are be decreased so the goalie stays close
        // towards the net
        auto defense_area_deflation = Util::DynamicParameters->getAIConfig()->getGoalieTacticConfig()
        ...
        else
        {
            ...
            float radius = Util::DynamicParameters->getAIConfig()->getGoalieTacticConfig()
                               ->BlockConeBuffer()
                               ->value() +
