...
{
    World initial_world = ::Test::TestUtil::createBlankTestingWorld();
    Rectangle target_region = Rectangle(Point(1, 1), Point(0, 0));
    World update_world = ::Test::TestUtil::createBlankTestingWorld();
    update_world =
            ::Test::TestUtil::setBallPosition(update_world, Point(1, 0), Timestamp::fromSeconds(0));
    update_world.updateFieldGeometry(Field(10.0, 10.0, 10.0, 10.0, 10.0, 10.0, 10.0, Timestamp::fromSeconds(0)));
...
TEST(TacticUpdateVisitorTest, update_crease_defender_tactic)
{
    ...
    update_world =
            ::Test::TestUtil::setBallPosition(update_world, Point(1, 0), Timestamp::fromSeconds(0));
    update_world.updateFieldGeometry(Field(10.0, 10.0, 10.0, 10.0, 10.0, 10.0, 10.0, Timestamp::fromSeconds(0)));
    CreaseDefenderTactic tactic = CreaseDefenderTactic(initial_world.field(), initial_world.ball(), initial_world.friendlyTeam(), initial_world.enemyTeam(), CreaseDefenderTactic::LEFT);
...
TEST(TacticUpdateVisitorTest, update_defense_shadow_enemy)
{
    ...
    update_world =
            ::Test::TestUtil::setBallPosition(update_world, Point(1, 0), Timestamp::fromSeconds(0));
    update_world.updateFieldGeometry(Field(10.0, 10.0, 10.0, 10.0, 10.0, 10.0, 10.0, Timestamp::fromSeconds(0)));
    DefenseShadowEnemyTactic tactic = DefenseShadowEnemyTactic(initial_world.field(), initial_world.friendlyTeam(), initial_world.enemyTeam(), initial_world.ball(), true, 10.0);
    EXPECT_EQ(tactic.getField(), initial_world.field());
    EXPECT_EQ(tactic.getEnemyTeam(), initial_world.enemyTeam());
    EXPECT_EQ(tactic.getFriendlyTeam(), initial_world.friendlyTeam());
    EXPECT_EQ(tactic.getBall(), initial_world.ball());
...
TEST(TacticUpdateVisitorTest, update_goalie_tactic)
{
    ...
    GoalieTactic tactic = GoalieTactic(initial_world.ball(), initial_world.field(), initial_world.friendlyTeam(), initial_world.enemyTeam());
    EXPECT_EQ(tactic.getField(), initial_world.field());
    EXPECT_EQ(tactic.getEnemyTeam(), initial_world.enemyTeam());
    EXPECT_EQ(tactic.getFriendlyTeam(), initial_world.friendlyTeam());
    EXPECT_EQ(tactic.getBall(), initial_world.ball());
...
TEST(TacticUpdateVisitorTest, update_penalty_kick_tactic)
{
    ...
    PenaltyKickTactic tactic = PenaltyKickTactic(initial_world.ball(), initial_world.field(), std::nullopt, true);
    EXPECT_EQ(tactic.getField(), initial_world.field());
    EXPECT_EQ(tactic.getBall(), initial_world.ball());
    TacticWorldParamsUpdateVisitor visitor = TacticWorldParamsUpdateVisitor(update_world);
    tactic.accept(visitor);
...
TEST(TacticUpdateVisitorTest, update_receiver_tactic)
{
    ...
    update_world.updateFieldGeometry(Field(10.0, 10.0, 10.0, 10.0, 10.0, 10.0, 10.0, Timestamp::fromSeconds(0)));
    Pass pass({0, 0}, {0, -1}, 2.29, Timestamp::fromSeconds(5));
    ReceiverTactic tactic = ReceiverTactic(initial_world.field(), initial_world.friendlyTeam(), initial_world.enemyTeam(), pass, initial_world.ball(), false);
    EXPECT_EQ(tactic.getField(), initial_world.field());
    EXPECT_EQ(tactic.getEnemyTeam(), initial_world.enemyTeam());
    EXPECT_EQ(tactic.getFriendlyTeam(), initial_world.friendlyTeam());
    EXPECT_EQ(tactic.getBall(), initial_world.ball());
...
TEST(TacticUpdateVisitorTest, update_shadow_enemy_tactic)
{
    ...
    update_world =
            ::Test::TestUtil::setBallPosition(update_world, Point(1, 0), Timestamp::fromSeconds(0));
    update_world.updateFieldGeometry(Field(10.0, 10.0, 10.0, 10.0, 10.0, 10.0, 10.0, Timestamp::fromSeconds(0)));
    ShadowEnemyTactic tactic = ShadowEnemyTactic(initial_world.field(), initial_world.friendlyTeam(), initial_world.enemyTeam(), true, initial_world.ball(), 10.0, false);
    EXPECT_EQ(tactic.getField(), initial_world.field());
    EXPECT_EQ(tactic.getEnemyTeam(), initial_world.enemyTeam());
    EXPECT_EQ(tactic.getFriendlyTeam(), initial_world.friendlyTeam());
    EXPECT_EQ(tactic.getBall(), initial_world.ball());
...
TEST(TacticUpdateVisitorTest, update_shoot_goal_tactic)
{
    ...
    ShootGoalTactic tactic =
            ShootGoalTactic(initial_world.field(), initial_world.friendlyTeam(), initial_world.enemyTeam(), initial_world.ball(), Angle::zero(), std::nullopt, false);
    EXPECT_EQ(tactic.getField(), initial_world.field());
    EXPECT_EQ(tactic.getEnemyTeam(), initial_world.enemyTeam());
    EXPECT_EQ(tactic.getFriendlyTeam(), initial_world.friendlyTeam());
    EXPECT_EQ(tactic.getBall(), initial_world.ball());
