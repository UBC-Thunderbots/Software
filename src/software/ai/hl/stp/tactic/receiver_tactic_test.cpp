...
TEST(ReceiverTacticTest, robot_at_receive_position_pass_started_goal_open_angle_feasible)
{
    ...
    EXPECT_GT(move_action->getDestination().y(), -0.1);
    EXPECT_LT(move_action->getDestination().y(), 0.2);

    EXPECT_LT(move_action->getFinalOrientation().toDegrees(), -1);
    EXPECT_GT(move_action->getFinalOrientation().toDegrees(), -90);

    EXPECT_EQ(DribblerMode::OFF, move_action->getDribblerMode());
    ASSERT_TRUE(move_action->getAutoChipOrKick().auto_chip_kick_mode ==
                AutoChipOrKickMode::AUTOKICK);
    EXPECT_EQ(move_action->getAutoChipOrKick().autokick_speed_m_per_s,
