...
DefensePlayFSMBase::DefensePlayFSMBase(std::shared_ptr<TbotsProto::AiConfig> ai_config_ptr)
    : PlayFSM<DefensePlayBaseControlParams>(ai_config_ptr), crease_defenders({}), pass_defenders({})
{
}
...
void DefensePlayFSMBase::setUpCreaseDefenders(int num_crease_defenders)
{
    ...
    std::generate(crease_defenders.begin(), crease_defenders.end(),
                  [this]() { return std::make_shared<CreaseDefenderTactic>(ai_config_ptr); });
}
...
void DefensePlayFSMBase::setUpPassDefenders(int num_pass_defenders)
{
    ...
    std::generate(pass_defenders.begin(), pass_defenders.end(),
                  [this]() { return std::make_shared<PassDefenderTactic>(ai_config_ptr); });
}
