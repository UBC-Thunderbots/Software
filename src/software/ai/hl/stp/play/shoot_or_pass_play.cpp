...
bool ShootOrPassPlay::isApplicable(const World &world) const
{
    bool use_shoot_or_pass_instead_of_shoot_or_chip =
        Util::DynamicParameters->getAIConfig()->getHighLevelStrategyConfig()
...
void ShootOrPassPlay::getNextTactics(TacticCoroutine::push_type &yield)
{
    ...
    Angle min_open_angle_for_shot =
        Angle::fromDegrees(Util::DynamicParameters->getAIConfig()->getShootOrPassPlayConfig()
    ...
    double abs_min_pass_score =
        Util::DynamicParameters->getAIConfig()->getShootOrPassPlayConfig()->AbsMinPassScore()->value();
    double pass_score_ramp_down_duration =
        Util::DynamicParameters->getAIConfig()->getShootOrPassPlayConfig()
