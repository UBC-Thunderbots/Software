...
EnemyBallPlacementPlayFSM::EnemyBallPlacementPlayFSM(TbotsProto::AiConfig ai_config)
    : ai_config(ai_config),
      ...
      distance_to_keep(ENEMY_BALL_PLACEMENT_DISTANCE_METERS + 2 * ROBOT_MAX_RADIUS_METERS),
...
{
    return (event.common.world_ptr->ball().position() - placement_point).length() < nearly_placed_threshold;
}
...
void EnemyBallPlacementPlayFSM::avoid(const Update& event)
{
    ...
    for (auto const &robot : world_ptr->friendlyTeam().getAllRobots()) {
        if (contains(stadium, robot.position())) {
            Vector lateral_positioning_vector = placement_point_to_ball.perpendicular().normalize();
            Vector longitudinal_positioning_vector = (robot.position() - placement_point).project(placement_point_to_ball);
            Point p1 = placement_point + longitudinal_positioning_vector + lateral_positioning_vector * distance_to_keep;
            Point p2 = placement_point + longitudinal_positioning_vector - lateral_positioning_vector * distance_to_keep;
            Point destination = distance(p1, Point(0, 0)) < distance(p2, Point(0, 0)) ? p1 : p2;
            move_tactics[idx]->updateControlParams(destination, robot.orientation(), 0);
        } else {
            move_tactics[idx]->updateControlParams(robot.position(), robot.orientation(), 0);
        }
