...
def test_shoot_or_chip_play_wrapper(simulated_test_runner):
    ...
    def test_shoot_or_chip_play(always_validation_sequence_set, eventually_validation_sequence_set):
    ...
    if not simulated_test_runner.aggregate:
        print('Running Invariant Testing')
        test_shoot_or_chip_play([[]], [[]])
    else:
        tests = 0
        fail = 0
        ...
        print(f"{tests} Tests run. {fail} failed")
