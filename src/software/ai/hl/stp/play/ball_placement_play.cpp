...
BallPlacementPlay::BallPlacementPlay(std::shared_ptr<const PlayConfig> config) : Play(config) {}
...
void BallPlacementPlay::getNextTactics(TacticCoroutine::push_type &yield, const World &world)
{
    auto place_ball_tactic = std::make_shared<DribbleTactic>();
    place_ball_tactic->updateControlParams(world.gameState().getBallPlacementPoint(), std::nullopt);
    ...
    do
    {
        ...
        yield({place_ball_tactic, stop_tactic_2, stop_tactic_3, stop_tactic_4, stop_tactic_5,
               stop_tactic_6});
    } while (true);
}
