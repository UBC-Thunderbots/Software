...
TEST_F(EnemyFreekickPlayTest, test_enemy_free_kick_play)
{
    ...
    std::vector<ValidationFunction> terminating_validation_functions = {
        [](std::shared_ptr<World> world_ptr, ValidationCoroutine::push_type& yield) {
            ...
            // enemy robot performing the free kick, based on where the enemy robots are initialized in the test.
            ...
            // One Friendly robot defending the exterior of defense box to the right of the goalie
            Point goalie_position = world_ptr->friendlyTeam().goalie()->position();
            Rectangle crease_defender_rect(goalie_position, Point(goalie_position.x() + 0.2, goalie_position.y() - 0.2));
...
TEST_F(EnemyFreekickPlayTest, test_enemy_free_kick_close_to_net) {
    ...
    std::vector<ValidationFunction> terminating_validation_functions = {
            [](std::shared_ptr<World> world_ptr, ValidationCoroutine::push_type& yield) {
                ...
                // One Friendly robot defending the exterior of defense box to the left of the goalie
                Point goalie_position = world_ptr->friendlyTeam().goalie()->position();
                Rectangle crease_defender_rect(Point(goalie_position.x(), goalie_position.y() + 0.3), Point(goalie_position.x() + 0.3, goalie_position.y()));
    ...
    std::vector<ValidationFunction> non_terminating_validation_functions = {
            [](std::shared_ptr<World> world_ptr, ValidationCoroutine::push_type& yield) {
                ...
                // Due to tight space between defense area and the robot performing the enemy free kick we wait longer than in the other tests
...
TEST_F(EnemyFreekickPlayTest, test_enemy_free_kick_chipper_robots_close_to_net) {
    ...
    std::vector<ValidationFunction> terminating_validation_functions = {
            [](std::shared_ptr<World> world_ptr, ValidationCoroutine::push_type& yield) {
                ...
                // One Friendly robot defending the exterior of defense box to the left of the goalie
                Point goalie_position = world_ptr->friendlyTeam().goalie()->position();
                Rectangle crease_defender_rect(Point(goalie_position.x(), goalie_position.y() + 0.5), Point(goalie_position.x() + 0.3, goalie_position.y()));
                robotInPolygon(2, crease_defender_rect, world_ptr, yield);
            }};
