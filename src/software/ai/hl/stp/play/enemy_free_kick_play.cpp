                                       ...
                                       const World &world)
{
    ...
    do
    {
        ...
        {
            auto crease_defender_centre = std::make_shared<CreaseDefenderTactic>(
                ai_config.robot_navigation_obstacle_config());
        ...
        auto num_unassigned_robots =
            world.friendlyTeam().numRobots() - 2 - num_crease_defenders;
        ...
        while (num_unassigned_robots > 0)
        {
            ...
            if (enemy_threats[target_enemy_counter].robot.position().x() >=
                world.ball().position().x() + 1)
            {
            ...
            Point block_pass_point      = enemy_position + enemy_to_ball_vector.normalize(
