                                       ...
                                       const World &world)
{
    ...
    do
    {
        ...
        std::vector<EnemyThreat> enemy_threats = getAllEnemyThreats(
            world.field(), world.friendlyTeam(), world.enemyTeam(), world.ball(), false);
        std::queue defense_assignments = getAllDefenderAssignments(
            enemy_threats, world.field(), world.ball(),
        ...
        defense_play->updateControlParams(
            defense_assignments, TbotsProto::MaxAllowedSpeedMode::PHYSICAL_LIMIT);
        ...
        if (assign_pass_defender)
        {
            ...
            if (tactics_to_return.size() > 0 && tactics_to_return[0].size() > 0 &&
                assign_pass_defender)
            {
                tactics_to_return[0].insert(tactics_to_return[0].begin() + 1,
                                            block_free_kicker);
            }
