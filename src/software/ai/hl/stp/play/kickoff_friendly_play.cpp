...
bool KickoffFriendlyPlay::invariantHolds(const World &world) const
{
    ...
    return (!world.gameState().isHalted() && !world.gameState().isStopped() &&
                                         ...
                                         const World &world)
{
    ...
    // specific tactics for kickoff robots (special motion constraints for moving in
    // centre circle)
    ...
    std::vector<EighteenZoneId> kickoff_pass_zones = {
        EighteenZoneId::ZONE_7, EighteenZoneId::ZONE_10, EighteenZoneId::ZONE_9,
        EighteenZoneId::ZONE_12};

    auto update_wingers = [&](Pass left, Pass right) {
        std::get<0>(winger_tactics)
            ->updateControlParams(left.receiverPoint(), left.receiverOrientation(), 0.0,
                                  MaxAllowedSpeedMode::PHYSICAL_LIMIT);

        std::get<1>(winger_tactics)
            ->updateControlParams(right.receiverPoint(), right.receiverOrientation(), 0.0,
                                  MaxAllowedSpeedMode::PHYSICAL_LIMIT);
    };
    ...
    PassWithRating best_pass_and_score_so_far = pass_eval.getBestPassInZones(
        std::unordered_set<Zones>(kickoff_pass_zones.begin(), kickoff_pass_zones.end()));

    do
    {
        PassWithRating pass1 =
            pass_generator.generatePassEvaluation(world).getBestPassInZones(
                {kickoff_pass_zones[0], kickoff_pass_zones[1]});
        PassWithRating pass2 =
            pass_generator.generatePassEvaluation(world).getBestPassInZones(
                {kickoff_pass_zones[2], kickoff_pass_zones[3]});
        update_wingers(pass1.pass, pass2.pass);

        auto current_best_pass_with_rating =
            (pass1.rating >= pass2.rating) ? pass1 : pass2;
        ...
        // to prevent the kicker from being paralyzed at kickoff, we accept a pass
        // threshold where we lock on to a less favourable pass
        ...
        Vector ball_to_passer =
            best_pass.value().receiverPoint() - world.ball().position();
        ...
        yield({{kickoff_move_tactic, std::get<0>(winger_tactics),
                std::get<1>(winger_tactics)}});
