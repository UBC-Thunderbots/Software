...
void DefensePlay::getNextTactics(TacticCoroutine::push_type &yield)
{
    ...
    do
    {
        ...
        goalie_tactic->updateWorldParams(world.ball(), world.field(), friendly_team_for_goalie,
                                    world.enemyTeam());
        ...
        shoot_goal_tactic->updateWorldParams(world.field(), world.friendlyTeam(), world.enemyTeam(), world.ball());
        ...
        for (auto crease_defender_tactic : crease_defender_tactics)
        {
            crease_defender_tactic->updateWorldParams(world.ball(), world.field(),
                                                 world.friendlyTeam(), world.enemyTeam());
        ...
        if (enemy_threats.size() > 0) {
            defense_shadow_enemy_tactic->updateWorldParams(world.field(),
                                                      world.friendlyTeam(), world.enemyTeam(), world.ball());
            ...
            if(world.enemyTeam().numRobots() > 0) {
                ...
                move_tactics[0]->updateControlParams(block_point, nearest_enemy_robot.orientation() + Angle::half(),
                                              0.0);
                move_tactics[1]->updateControlParams(block_point, nearest_enemy_robot.orientation() + Angle::half(),
                                              0.0);
        ...
        if (enemy_threats.size() > 1) {
            shadow_enemy_tactic->updateWorldParams(world.field(), world.friendlyTeam(), world.enemyTeam(), world.ball());
            shadow_enemy_tactic->updateControlParams(enemy_threats.at(0), ROBOT_MAX_RADIUS_METERS * 3);
