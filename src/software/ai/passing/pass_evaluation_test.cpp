...
TEST(PassEvaluation, best_pass_over_entire_field)
{
    ...
    for (unsigned zone_id = 1; zone_id <= pitch_division->getTotalNumberOfZones();
         zone_id++)
    {
        passes_with_rating.emplace_back(
    ...
    auto pass_eval =
        PassEvaluation(pitch_division, passes_with_rating, Timestamp::fromSeconds(10));
...
TEST(PassEvaluation, best_pass_in_zones)
{
         ...
         zone_id++)
    {
        ...
        passes_with_rating.emplace_back(
            PassWithRating{std::move(Pass(Point(0, 0), 0)), 0.05 * zone_id});
    }
