...
struct PlaySelectionFSM
{
    ...
    auto operator()()
    {
        ...
        const auto gameStateStopped_G      = SMLGuard<&PlaySelectionFSM::gameStateStopped>{this};
        const auto gameStateHalted_G       = SMLGuard<&PlaySelectionFSM::gameStateHalted>{this};
        const auto gameStatePlaying_G      = SMLGuard<&PlaySelectionFSM::gameStatePlaying>{this};
        const auto gameStateSetupRestart_G = SMLGuard<&PlaySelectionFSM::gameStateSetupRestart>{this};
        ...
        const auto setupOffensePlay_A      = SMLAction<&PlaySelectionFSM::setupOffensePlay>{this};
        ...
        return make_transition_table(
            ...
            *Halt_S   + Update_E[gameStateStopped_G]      / setupStopPlay_A                      = Stop_S,
            ...
            // Check for transitions to other states, if not then default to running the
            // current play
            SetPlay_S + Update_E[gameStateHalted_G]       / (resetSetPlay_A, setupHaltPlay_A)    = Halt_S,
            SetPlay_S + Update_E[gameStateStopped_G]      / (resetSetPlay_A, setupStopPlay_A)    = Stop_S,
            SetPlay_S + Update_E[gameStatePlaying_G]      / (resetSetPlay_A, setupOffensePlay_A) = Playing_S,
