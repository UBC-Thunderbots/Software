...
TEST_F(TestGlobalPathPlanner, test_both_defence_areas_blocked_starting_and_ending_in_blocked_areas)
{
    ...
    EXPECT_LE(path->getEndPoint().x(), world.field().enemyDefenseArea().negXNegYCorner().x());
    ...
    TestUtil::checkPathDoesNotExceedBoundingBox(path_points, bounding_box);
    // since the starting point is inside an obstacle, we need to consider the path from the second point onwards
    TestUtil::checkPathDoesNotIntersectObstacle({path_points.begin()+1, path_points.end()}, obstacles);
}

TEST_F(TestGlobalPathPlanner, test_both_defense_areas_and_centre_circle_blocked_starting_and_ending_in_blocked_areas)
{
    Point start {-5.6, 0}, dest{5.6, 0.1};
    
    std::set<MotionConstraint> constraints = { 
        MotionConstraint::FRIENDLY_DEFENSE_AREA, MotionConstraint::ENEMY_DEFENSE_AREA, MotionConstraint::CENTER_CIRCLE 
    };
    std::vector<ObstaclePtr> obstacles = obstacle_factory.createFromMotionConstraints(constraints, world);
    ...
    // we are differentiating between defense area obstacles and the center circle obstacles because the lost resolution
    // means that we interfere with the center circle while path planning (but not to a problematic degree). THis won't
    // be a problem since obstacles are inflated by the RobotNavigationObstacleFactory
    std::vector<ObstaclePtr> defense_area_obstacles = obstacle_factory.createFromMotionConstraints(
        { MotionConstraint::FRIENDLY_DEFENSE_AREA,MotionConstraint::ENEMY_DEFENSE_AREA }, world);
    TestUtil::checkPathDoesNotIntersectObstacle({path_points.begin()+1, path_points.end()}, defense_area_obstacles);
    std::vector<Polygon> centre_circle_obstacle = {  Rectangle(Point(-0.5, -0.5), Point(0.5, 0.5)) };
    TestUtil::checkPathDoesNotIntersectObstacle(path_points, centre_circle_obstacle);
}

TEST_F(TestGlobalPathPlanner, test_enemy_half_and_centre_circle_blocked_starting_in_blocked_area)
{
    Point start {3, 3}, dest {-3, -3};
    
    std::set<MotionConstraint> constraints = {
        MotionConstraint::ENEMY_HALF, MotionConstraint::CENTER_CIRCLE, MotionConstraint::FRIENDLY_DEFENSE_AREA
    };
    std::vector<ObstaclePtr> obstacles = obstacle_factory.createFromMotionConstraints(constraints, world);
