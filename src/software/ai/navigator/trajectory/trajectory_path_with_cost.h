...
class TrajectoryPathWithCost
{
   public:
    ...
    // The time and obstacle and velocity at which the trajectory first collides with an obstacle
    // first_collision_time_s is set to infinity and colliding_obstacle is nullptr
    // if the trajectory does not collide.
	// first_collision_velocity_m_s is initialized as 0.0 as if no collision happens then there shouldn't be a velocity
