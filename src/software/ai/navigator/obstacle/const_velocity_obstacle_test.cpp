...
static constexpr double MAX_OBSTACLE_TIME_HORIZON = 1.0;

TEST(ConstVelocityObstacleTest, circle_obstacle_contains)
{
    Circle circle({2, 0}, 1);
    ObstaclePtr obstacle(std::make_shared<ConstVelocityObstacle<Circle>>(circle, Vector(0.5, 0), MAX_OBSTACLE_TIME_HORIZON));
    // Point barely inside obstacle
    Point point_1(1.01, 0);
    // Point barely outside obstacle
    Point point_2(3.01, 0);

    // 0 seconds into the future
    EXPECT_TRUE(obstacle->contains(point_1, 0.0));
    EXPECT_FALSE(obstacle->contains(point_2, 0.0));
    ...
    // After >MAX_OBSTACLE_TIME_HORIZON seconds, we should not predict the obstacles position
    EXPECT_FALSE(obstacle->contains(point_1, MAX_OBSTACLE_TIME_HORIZON + 1.0));
    EXPECT_TRUE(obstacle->contains(point_2, MAX_OBSTACLE_TIME_HORIZON + 1.0));
}

TEST(ConstVelocityObstacleTest, circle_obstacle_distance)
{
    Circle circle({0, 0}, 1.0);
    ObstaclePtr obstacle(std::make_shared<ConstVelocityObstacle<Circle>>(circle, Vector(1.0, 0), MAX_OBSTACLE_TIME_HORIZON));
    ...
    // After >MAX_OBSTACLE_TIME_HORIZON seconds, we should not predict the obstacles position
    EXPECT_EQ(obstacle->distance(point_1, MAX_OBSTACLE_TIME_HORIZON + 1.0), 1.0);
    EXPECT_EQ(obstacle->distance(point_2, MAX_OBSTACLE_TIME_HORIZON + 1.0), 0.0);
}
