...
{
    Circle circle({2, 0}, 1);
    ObstaclePtr obstacle(std::make_shared<ConstVelocityObstacle<Circle>>(
        circle, Vector(0.5, 0), MAX_OBSTACLE_TIME_HORIZON));
    ...
    // After >MAX_OBSTACLE_TIME_HORIZON seconds, we should not predict the obstacles
    // position
...
TEST(ConstVelocityObstacleTest, circle_obstacle_distance)
{
    ...
    ObstaclePtr obstacle(std::make_shared<ConstVelocityObstacle<Circle>>(
        circle, Vector(1.0, 0), MAX_OBSTACLE_TIME_HORIZON));
...
TEST(ConstVelocityObstacleTest, circle_obstacle_intersects)
{
    ...
    ObstaclePtr obstacle(
        std::make_shared<ConstVelocityObstacle<Circle>>(circle, Vector(1.0, 0), 3.0));
