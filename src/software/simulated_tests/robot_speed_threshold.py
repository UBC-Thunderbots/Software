...
class RobotSpeedThreshold(Validation):
    ...
    def get_validation_geometry(self, world) -> ValidationGeometry:
        ...
        for robot in world.friendly_team.team_robots:
            ...
            validation_centre_x = (robot_x + self.MILLIMETERS_PER_METER * self.speed_threshold * math.cos(robot_angle))
            validation_centre_y = (robot_y + self.MILLIMETERS_PER_METER * self.speed_threshold * math.sin(robot_angle))
            
            endpoints = self.get_validation_line_endpoints(validation_centre_x, validation_centre_y, robot_angle)
            start_x = endpoints[0]
            end_x = endpoints[1]
            start_y = endpoints[2]
            end_y = endpoints[3]
    ...
    def get_validation_line_endpoints(self, validation_centre_x, validation_centre_y, robot_angle):
        start_x = validation_centre_x - math.sin(robot_angle) * self.VALIDATION_LINE_SCALE_FACTOR
        end_x = validation_centre_x + math.sin(robot_angle) * self.VALIDATION_LINE_SCALE_FACTOR

        start_y = validation_centre_y + math.cos(robot_angle) * self.VALIDATION_LINE_SCALE_FACTOR
        end_y = validation_centre_y - math.cos(robot_angle) * self.VALIDATION_LINE_SCALE_FACTOR
