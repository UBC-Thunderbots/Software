...
void robotTooCloseDuringEnemyKickOff(std::shared_ptr<World> world_ptr, ValidationCoroutine::push_type& yield) {
            ...
            [](std::shared_ptr<World> world_ptr) {
                ...
                if (game_state.isTheirKickoff())
                {
                    ...
                    for (const auto& robot : world_ptr->friendlyTeam().getAllRobots())
                    {
                        ...
                        //Friendly robots must not be < 0.5 meters to the ball during enemy kick-off
                        if (abs_error > 0.025)
                        {
                            return false;
                        }
