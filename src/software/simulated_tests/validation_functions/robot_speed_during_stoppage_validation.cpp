...
void robotTooFastDuringStoppage(std::shared_ptr<World> world_ptr, ValidationCoroutine::push_type& yield) {
    auto robot_too_fast =
            [](std::shared_ptr<World> world_ptr) {
                ...
                if (game_state.isStopped())
                {
                    for (const auto& robot : world_ptr->friendlyTeam().getAllRobots())
                    {
                        ...
                        //Robots cannot exceed a speed of 1.5 m/s during stoppage as regulated by the SSL
