...
void robotTooCloseDuringEnemyFreeKick(std::shared_ptr<World> world_ptr, ValidationCoroutine::push_type& yield) {
    auto robot_too_close =
            [](std::shared_ptr<World> world_ptr) {
                GameState game_state = world_ptr->gameState();
                if (game_state.isTheirFreeKick() || game_state.isTheirDirectFree() || game_state.isTheirIndirectFree())
                {
                    ...
                    for (const auto& robot : world_ptr->friendlyTeam().getAllRobots())
                    {
                        ...
                        //Friendly robots must not be < 0.5 meters to the ball during enemy free kick
                        if (abs_error > 0.025)
                        {
                            return false;
                        }
