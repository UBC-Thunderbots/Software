...
@pytest.mark.parametrize(
    ...
    [(-2.0, -0.3), (-2.0, 0.3), (-1.0, 0.3), (-1.0, -0.3)],
)
def test_basic_movement(
    tbots_test_runner,robot_x_position, robot_y_position
):
    ...
    eventually_validation_sequence_set = [
        [
            RobotEventuallyEntersRegion(
                regions=[tbots.Circle(tbots.Point(robot_x_position, robot_y_position),0.2)]
            ),
        ]
    ]
    ...
    tbots_test_runner.run_test(eventually_validation_sequence_set=eventually_validation_sequence_set, test_timeout_s=5)
