...
{
    auto robot_in_polygon = [polygon](std::shared_ptr<World> world_ptr) {
    ...
    while (robot_in_polygon(world_ptr)<count)
    {
        std::stringstream ss_poly;
        ss_poly << polygon;

        yield("There were not " + std::to_string(count) + " robots simulataneously in " +
              ss_poly.str());
    }
}
