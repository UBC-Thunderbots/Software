...
ContinuousFunctionValidator::ContinuousFunctionValidator(
    ...
    : world_ptr(world), validation_function(validation_function),
    ...
    // inside the coroutine will not update properly when the pointer is updated.
      validation_sequence(boost::bind(
              &ContinuousFunctionValidator::executeAndCheckForFailuresWrapper, this, _1, world))
{
}
...
void ContinuousFunctionValidator::executeAndCheckForFailures() {
    ...
    if(!validation_sequence) {
        ...
        validation_sequence = ValidationCoroutine::pull_type(boost::bind(
                &ContinuousFunctionValidator::executeAndCheckForFailuresWrapper, this, _1, world_ptr));
    }
...
void ContinuousFunctionValidator::executeAndCheckForFailuresWrapper(ValidationCoroutine::push_type &yield,
