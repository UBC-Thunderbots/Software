...
TEST(ContinuousFunctionValidatorTest, test_validation_function_that_does_nothing_does_not_report_failure)
{
...
{
    // This validation_functions uses exceptions as a way for the test to observe it's internal state
    ValidationFunction validation_function = [](std::shared_ptr<World> world,
                                                ValidationCoroutine::push_type& yield) {
...
{
    // This validation_functions uses exceptions as a way for the test to observe it's internal state
    // The exception will not be reached until the 3rd function call
    ValidationFunction validation_function = [](std::shared_ptr<World> world,
                                                ValidationCoroutine::push_type& yield) {
        yield();
        yield();
