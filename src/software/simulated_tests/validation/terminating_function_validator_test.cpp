     ...
     test_validation_function_with_single_loop_succeeds_after_loop_termination)
{
    ...
    world->updateBallStateWithTimestamp(BallState(Point(-1, 0), Vector(0, 0)),
                                        Timestamp::fromSeconds(0));
    bool result = function_validator.executeAndCheckForSuccess();
    EXPECT_FALSE(result);

    world->updateBallStateWithTimestamp(BallState(Point(-0.1, 0), Vector(0, 0)),
                                        Timestamp::fromSeconds(0));
    result = function_validator.executeAndCheckForSuccess();
    EXPECT_FALSE(result);

    world->updateBallStateWithTimestamp(BallState(Point(0.05, 0), Vector(0, 0)),
                                        Timestamp::fromSeconds(0));
    result = function_validator.executeAndCheckForSuccess();
    EXPECT_TRUE(result);
}
     ...
     test_validation_function_with_two_loops_succeeds_after_both_loops_terminate_in_order)
{
    ...
    world->updateBallStateWithTimestamp(BallState(Point(-1, -1), Vector(0, 0)),
                                        Timestamp::fromSeconds(0));
    bool result = function_validator.executeAndCheckForSuccess();
    EXPECT_FALSE(result);
...
TEST(TerminatingFunctionValidatorTest, test_validation_function_with_gtest_statements)
{
    ...
    world->updateBallStateWithTimestamp(BallState(Point(0, 0), Vector(0, 0)),
                                        Timestamp::fromSeconds(0));
