...
HandheldControllerBackend::HandheldControllerBackend() : HandheldControllerBackend(std::make_shared<const HandheldControllerInputConfig>())
{
}
...
void HandheldControllerBackend::onValueReceived(ConstPrimitiveVectorPtr primitives_ptr)
{
    ...
    Robot robot = Robot(controller_input_config->RobotId()->value(), Point(0, 0), Vector(0, 0), Angle::zero(), AngularVelocity::zero(), Timestamp::fromSeconds(0));
