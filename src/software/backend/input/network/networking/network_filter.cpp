...
Ball NetworkFilter::getFilteredBallData(const std::vector<SSL_DetectionFrame> &detections)
{
    ...
    for (const auto &detection : detections)
    {
        for (const SSL_DetectionBall &ball : detection.balls())
        {
            ...
            // TODO remove Util::DynamicParameters as part of https://github.com/UBC-Thunderbots/Software/issues/960 
            bool ball_position_invalid =
                Util::DynamicParameters->getAIConfig()->getRefboxConfig()->MinValidX()->value() >
                    ball_detection.position.x() ||
                Util::DynamicParameters->getAIConfig()->getRefboxConfig()->MaxValidX()->value() <
                    ball_detection.position.x();
            bool ignore_ball =
                Util::DynamicParameters->getAIConfig()->getRefboxConfig()->IgnoreInvalidCameraData()->value() &&
    ...
    const std::vector<SSL_DetectionFrame> &detections)
{
    ...
    for (const auto &detection : detections)
    {
        ...
        // TODO remove Util::DynamicParameters as part of https://github.com/UBC-Thunderbots/Software/issues/960 
        if (!Util::DynamicParameters->getAIConfig()->getRefboxConfig()->FriendlyColorYellow()->value())
        {
