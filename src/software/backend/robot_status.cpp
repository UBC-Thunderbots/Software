...
TbotsRobotMsg convertRobotStatusToTbotsRobotMsg(RobotStatus* robot_status)
{
    ...
    break_beam_msg->set_break_beam_reading((float)robot_status->break_beam_reading);
    ...
    dribbler_status_msg->set_dribbler_rpm((float)robot_status->dribbler_speed);
    robot_msg.set_allocated_dribbler_status(dribbler_status_msg.release());

    auto power_status_msg = std::make_unique<PowerStatus>();
    power_status_msg->set_battery_voltage((float)robot_status->battery_voltage);
    power_status_msg->set_capacitor_voltage((float)robot_status->capacitor_voltage);
    ...
    temperature_status_msg->set_board_temperature((float)robot_status->board_temperature);
