 ...
 * ProtoMulticastListener<SSL::SSL_Referee> to abstract all ssl protobuf networking
...
class SSLProtoClient
{
   public:
     ...
     * received
     * @param received_referee_callback Callback for when a new SSL::SSL_Referee is
     * received
    ...
    explicit SSLProtoClient(
        ...
        std::function<void(SSL::SSL_Referee)> received_referee_callback,
   ...
   private:
    ...
    std::unique_ptr<ThreadedProtoMulticastListener<SSL_WrapperPacket>>
        ssl_vision_listener;
    std::unique_ptr<ThreadedProtoMulticastListener<SSL::SSL_Referee>>
