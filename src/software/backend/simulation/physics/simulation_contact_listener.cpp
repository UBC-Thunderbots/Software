...
void SimulationContactListener::PreSolve(b2Contact *contact, const b2Manifold *oldManifold) {
    if (contact->IsTouching())
    {
        ...
        if(auto ball_dribbler_pair = isBallDribblerContact(user_data_a, user_data_b))
        {
            // We always disable contacts between the ball and the dribbler so that the ball
            // can pass through the dribbler area. This is needed so that the ball can exist
            // inside the dribbling "zone", as well as pass through the dribbler to make contact
            // with the robot chicker
