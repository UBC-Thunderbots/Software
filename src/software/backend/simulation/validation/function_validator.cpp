...
FunctionValidator::FunctionValidator(const ValidationFunction &validation_function,
    ...
    // We need to provide the world in the coroutine function binding so that the wrapper function
    // has access to the correct variable context, otherwise the World inside the coroutine will
    // not update properly when the pointer is updated.
      validation_sequence(
          boost::bind(&FunctionValidator::executeAndCheckForSuccessWrapper, this, _1, world))
{
}
