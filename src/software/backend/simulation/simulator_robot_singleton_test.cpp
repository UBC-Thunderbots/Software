...
TEST(PhysicsRobotTest, test_create_firmware_robot_with_single_simulator_robot)
{
    ...
    Robot robot(7, Point(1.2, 0), Vector(-2.3, 0.2), Angle::fromRadians(-1.2), AngularVelocity::quarter(), Timestamp::fromSeconds(0));
    ...
    auto simulator_robot = std::make_shared<SimulatorRobot>(friendly_physics_robots.at(0));
...
{
    World world = ::Test::TestUtil::createBlankTestingWorld();
    Robot robot7(7, Point(1.2, 0), Vector(-2.3, 0.2), Angle::fromRadians(-1.2), AngularVelocity::quarter(), Timestamp::fromSeconds(0));
    Robot robot2(2, Point(0, -4.03), Vector(0, 1), Angle::fromRadians(0.3), AngularVelocity::half(), Timestamp::fromSeconds(0));
    world.mutableFriendlyTeam().updateRobots({robot7, robot2});

    auto physics_world = std::make_unique<PhysicsWorld>(world);
    auto friendly_physics_robots = physics_world->getFriendlyPhysicsRobots();
    ASSERT_EQ(2, friendly_physics_robots.size());
    auto simulator_robot_7 = std::make_shared<SimulatorRobot>(friendly_physics_robots.at(0));
    ...
    // The firmware functions should now return the data for simulator_robot_2, even though
    // we didn't need to create a new FirmwareRobot_t
    auto simulator_robot_2 = std::make_shared<SimulatorRobot>(friendly_physics_robots.at(1));
