...
Simulator::Simulator(const World& world) : physics_world(world)
{
    for (auto physics_robot : physics_world.getFriendlyPhysicsRobots())
    {
        ...
        std::unique_ptr<PrimitiveManager, PrimitiveManagerDeleter> primitive_manager(app_primitive_manager_create(), PrimitiveManagerDeleter());
    ...
    firmware_world = std::unique_ptr<FirmwareWorld_t, FirmwareWorldDeleter>(firmware_world_raw, FirmwareWorldDeleter());
}
...
void Simulator::setPrimitives(ConstPrimitiveVectorPtr primitives)
{
    if(primitives) {
        ...
        for (auto iter = simulator_robots.begin(); iter != simulator_robots.end(); iter++)
        {
            ...
            auto primitive_iter = std::find_if(primitive_vector.begin(), primitive_vector.end(), [&simulator_robot](const auto& p) {
            ...
            if(primitive_iter != primitive_vector.end()) {
                ...
                unsigned int primitive_index = static_cast<unsigned int>(radio_primitive.prim_type);
                ...
                SimulatorRobotSingleton::setSimulatorRobot(simulator_robot);
                app_primitive_manager_startNewPrimitive(primitive_manager_raw_ptr, firmware_world.get(), primitive_index, &primitive_params);
            }
        }
    }
}
