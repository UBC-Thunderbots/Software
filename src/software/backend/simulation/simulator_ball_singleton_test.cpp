...
TEST(SimulatorBallSingletonTest, test_create_firmware_ball_with_single_simulator_ball) {
    ...
    world.mutableBall() = Ball(Point(0.4, 0), Vector(-1.3, 2.01), Timestamp::fromSeconds(0));
    auto physics_world = std::make_unique<PhysicsWorld>(world);
    auto simulator_ball = std::make_shared<SimulatorBall>(physics_world->getPhysicsBall());
...
TEST(SimulatorBallSingletonTest, test_change_simulator_ball) {
    ...
    world_1.mutableBall() = Ball(Point(0.4, 0), Vector(-1.3, 2.01), Timestamp::fromSeconds(0));
    auto physics_world_1 = std::make_unique<PhysicsWorld>(world_1);
    auto simulator_ball_1 = std::make_shared<SimulatorBall>(physics_world_1->getPhysicsBall());
    ...
    auto simulator_ball_2 = std::make_shared<SimulatorBall>(physics_world_2->getPhysicsBall());
