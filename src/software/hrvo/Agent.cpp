...
namespace hrvo {
	...
	void Agent::computePreferredVelocity()
	{
		...
		float distToGoal = sqrt(sqr(distVectorToGoal.getX()) + sqr(distVectorToGoal.getY()));
		// d = (Vf^2 - Vi^2) / 2a
		float startLinearDecelerationDistance = std::abs((sqr(speedAtGoal) - sqr(prefSpeed_)) / (2*maxAccel_));
		...
		if (distToGoal < startLinearDecelerationDistance)
		{
			...
			float currPrefSpeed = sqrt(sqr(speedAtGoal) + 2 * maxAccel_ * distToGoal);
