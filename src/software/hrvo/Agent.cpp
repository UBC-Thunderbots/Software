...
void Agent::computePreferredVelocity()
{
    ...
    auto distToGoal = static_cast<float>(std::sqrt(sqr(distVectorToGoal.getX()) + sqr(distVectorToGoal.getY())));
    // d = (Vf^2 - Vi^2) / 2a
    double startLinearDecelerationDistance = std::abs((sqr(speedAtGoal) - sqr(prefSpeed_)) / (2*maxAccel_));

    if (distToGoal < startLinearDecelerationDistance)
    {
        // velocity given linear deceleration, distance away from goal, and desired final speed
        auto currPrefSpeed = static_cast<float>(std::sqrt(sqr(speedAtGoal) + 2 * maxAccel_ * distToGoal));
