...
class HRVOTest : public ::testing::Test 
{
 public:
        ...
        : simulator()
    {
    	...
    	simulator.setAgentDefaults(3.f, 30, ROBOT_RADIUS * RADIUS_SCALE, 0.02f, /*prefSpeed=*/3.5f, /*maxSpeed=*/4.825f, /*uncertaintyOffset=*/0.f, /*maxAccel=*/3.28f);
    }
    ...
    void run_simulator()
    {
        ...
        do {
            ...
            for (unsigned int robot_id = 0; robot_id < num_robots; robot_id++)
            {
                ...
                output_file << frame << "," << time << "," << std::to_string(computationTime.count()) << "," << robot_id << "," << robot_radius[robot_id] << "," << curr_robot_pos.getX() << "," << curr_robot_pos.getY() << "," << velocity_x << "," << velocity_y << "," << speed << "," << has_collided << "," << simulator.getAgentPrefVelocity(robot_id).getX() << "," << simulator.getAgentPrefVelocity(robot_id).getY() << std::endl;
            }
            frame++;
...
TEST_F(HRVOTest, 1_robot_two_goals) {
    simulator.addAgent(Vector2(-4.f, 0.f), simulator.addGoalPositions({Vector2(4.f, 0.f), Vector2(-4.f, 0.f)}));
}
...
TEST_F(HRVOTest, 1_robot_moving_in_square_with_final_speed) {
    simulator.addAgent(Vector2(-4.f, -4.f), simulator.addGoalPositions({Vector2(4.f, -4.f), Vector2(4.f, 4.f), Vector2(-4.f, 4.f), Vector2(-4.f, -4.f)}, {2.f, 2.f, 2.f, 0.f}));
}
