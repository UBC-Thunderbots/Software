...
TEST(AngleSegmentTest, get_delta_zero_angle)
{
    ...
    EXPECT_EQ(180, angle_seg.getDelta().toDegrees());
}
...
EXPECT_EQ(90, angle_seg.getDelta().toDegrees());
}
...
{
AngleSegment angle_seg = AngleSegment(Angle::fromDegrees(-90), Angle::fromDegrees(-45));

EXPECT_EQ(45, angle_seg.getDelta().toDegrees());
}
