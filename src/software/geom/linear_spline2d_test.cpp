...
TEST(LinearSpline2dTest, test_spline_with_three_points_has_two_segments)
{
...
TEST(LinearSpline2dTest, test_spline_with_three_points_has_correct_segments)
{
    LinearSpline2d spline({Point(1, 2), Point(2, 3), Point(0, -1)});
    std::vector<SplineSegment2d> spline_segments = spline.getSplineSegments();
    EXPECT_EQ(spline_segments.size(), 2);
    ...
    EXPECT_EQ(spline_segments[0].getPolynomial(),
              segment1);  // make sure the polynomial matches what we expect (a linear
                          // Polynomial2d that goes through the first and second points)
    EXPECT_EQ(spline_segments[0].getParametrizationStartVal(),
              0);  // make sure the first segment starts at t=0.0
    EXPECT_EQ(spline_segments[0].getParametrizationEndVal(),
              1.0 / 2.0);  // and ends at t=0.5

    Polynomial1d segment2_x({{1.0 / 2.0, 2}, {1, 0}});
    Polynomial1d segment2_y({{1.0 / 2.0, 3}, {1, -1}});
    Polynomial2d segment2(segment2_x, segment2_y);
    EXPECT_EQ(spline_segments[1].getPolynomial(),
              segment2);  // make sure the polynomial matches what we expect (a linear
                          // Polynomial2d that goes through the second and third points)
    EXPECT_EQ(spline_segments[1].getParametrizationStartVal(),
              1.0 / 2.0);  // make sure the second segment starts at t=0.5
    EXPECT_EQ(spline_segments[1].getParametrizationEndVal(), 1);  // and ends at t=1.0
}

TEST(LinearSpline2dTest, test_spline_with_two_points_has_one_segment)
{
...
TEST(LinearSpline2dTest, test_spline_with_two_points_has_correct_segment)
{
    LinearSpline2d spline({Point(4.5, 3.2), Point(1.1, 3.34)});
    std::vector<SplineSegment2d> spline_segments = spline.getSplineSegments();
    EXPECT_EQ(spline_segments.size(), 1);
    ...
    EXPECT_EQ(spline_segments[0].getPolynomial(),
              segment1);  // make sure the polynomial matches what we expect (a linear
                          // Polynomial2d that goes through both points)
    EXPECT_EQ(spline_segments[0].getParametrizationStartVal(),
              0);  // make sure the single segment starts at t=0.0
    EXPECT_EQ(spline_segments[0].getParametrizationEndVal(), 1);  // and ends at t=1.0
}

TEST(LinearSpline2dTest, test_spline_with_one_point_has_zero_segments)
{
