...
PrimitiveGenerator::PrimitiveGenerator(std::shared_ptr<const HandheldControllerInputConfig> controller_input_config) : controller_input_config(controller_input_config) {
}
...
std::unique_ptr<Primitive> PrimitiveGenerator::createPrimitveFromControllerInput(
    ...
    if(controller_input.isKickButtonPressed()) {
        ...
        auto kick_primitive = std::make_unique<KickPrimitive>(robot_id, Point(0, 0), Angle::zero(), kick_speed);
        ...
        auto chip_primitive = std::make_unique<ChipPrimitive>(robot_id, Point(0, 0), Angle::zero(), chip_distance);
        return chip_primitive;
    }else {
        unsigned int dribbler_rpm = controller_input.isDribblerButtonPressed() ? controller_input_config->DribblerRpm()->value() : 0;
        double x_velocity = controller_input.getLinearMotionX() * controller_input_config->MaxLinearSpeed()->value();
        double y_velocity = controller_input.getLinearMotionY() * controller_input_config->MaxLinearSpeed()->value();
        double angular_velocity = controller_input.getAngularMotion() * controller_input_config->MaxAngularSpeed()->value();
        auto direct_velocity_primitive = std::make_unique<DirectVelocityPrimitive>(robot_id, x_velocity, y_velocity, angular_velocity, dribbler_rpm);
