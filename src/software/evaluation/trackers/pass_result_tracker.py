...
from software.evaluation.events.pass_result import (
    PassResultType,
    TrackedPassResult,
)
from software.evaluation.events.tracked_event import (
    EventType,
    Team
)
...
class PassResultTracker(PassTracker):
    def __init__(
        ...
        **kwargs,
        ...
        self.logged_passes_map: dict[int, list[TrackedPassResult]] = {
    ...
    def write_event(self, event_type: EventType) -> None:
        ...
        pass_result = self._log_pass_result(
          pass_result_type=PassResultType.RESULT_0S,
          pass_=self.curr_pass
        )

        self.logged_passes_map[PassResultsConstants.INTERVALS_S[0]].append(pass_result)
    ...
    def _log_pass_result(
        self, pass_result_type: PassResultType, pass_: Pass
    ) -> TrackedPassResult:
        ...
        pass_result = TrackedPassResult.from_world_and_pass(
    ...
    def _log_if_over_interval(
        self, tracked_pass: TrackedPassResult, interval: float, pass_result_type: PassResultType
    ) -> bool:
        ...
        if (
            ...
            - tracked_pass.pass_event.world_state.timestamp
            > interval
        ):
