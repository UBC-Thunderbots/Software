...
bool PrimitiveExecutor::isAngularTrajectoryNew(const BangBangTrajectory1DAngular& new_trajectory) const
{
...
void PrimitiveExecutor::updatePrimitive(const TbotsProto::Primitive& primitive_msg)
{
    ...
    if (current_primitive_.has_move())
    {
        ...
        if (isLateralTrajectoryNew(new_trajectory_path))
        {
            ...
            x_pid.reset();
            y_pid.reset();
            LOG(INFO) << "new lateral trajectory accepted";
        }

        const BangBangTrajectory1DAngular new_angular_trajectory = createAngularTrajectoryFromParams(
...
{
    Vector target_velocity = trajectory_path_->getVelocity(time_since_linear_trajectory_creation_.toSeconds());
        ...
        case TbotsProto::Primitive::kMove:
        {
            ...
            if (!trajectory_path_.has_value() || !angular_trajectory_.has_value())
            {
                const auto prim = createDirectControlPrimitive(
                    Vector(), AngularVelocity(), 0.0, TbotsProto::AutoChipOrKick());
                LOG(INFO)
                    << "Not moving because trajectory_path_ has value " << trajectory_path_.has_value() << " angular has " << angular_trajectory_.has_value();
                return std::make_unique<TbotsProto::DirectControlPrimitive>(
                    prim->direct_control());
            }
            LOG(INFO) << "handle kmove start";
            ...
            {
                target_linear_velocity = globalToLocalVelocity({x_pid_close.step(error_lateral.x()), y_pid_close.step(error_lateral.y())}, orientation_);
                LOG(INFO) << "Y: close to target, doing pure PID with effort " << target_linear_velocity.y();
                LOG(INFO) << "X: close to target, doing pure PID with effort " << target_linear_velocity.x();
            } else
            {
                // FEEDFORWARD velocities from the trajectory
                const Vector trajectory_linear_velocity = getTargetLinearVelocity();
                error_lateral = trajectory_path_->getPosition(time_since_linear_trajectory_creation_.toSeconds()) - position_;
                const Vector pid_effort_lateral(x_pid.step(error_lateral.x()), y_pid.step(error_lateral.y()));
                LOG(INFO) << "Y: far from target, PID control effort: " << pid_effort_lateral.y() << " FF: " << trajectory_linear_velocity.y();
                LOG(INFO) << "X: far from target, PID control effort: " << pid_effort_lateral.x() << " FF: " << trajectory_linear_velocity.x();
            ...
            if (error_angular.abs().toDegrees() < ANGULAR_PURE_PID_THRESHOLD_DEGREES)
            {
                ...
                LOG(INFO) << "ANG: close to target, doing pure PID with effort: " << target_angular_velocity.toDegrees();
            } else
            {
                // FEEDFORWARD velocities from the trajectory
            ...
            target_linear_velocity = target_linear_velocity.normalize(std::min(target_linear_velocity.length(), static_cast<double>(robot_constants_.robot_max_speed_m_per_s)));
