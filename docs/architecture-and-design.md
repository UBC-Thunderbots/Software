...
The `Theta Star Path Planner` is a path planner that determines a series of points from the start and destination, such that none of the points are in an obstacle and none of the line segments between adjacent points cross an obstacle, optimizing for the shortest total path length.

To explain how Theta Star (Theta\*) works, it's helpful to briefly explain what A Star (A\*) is. A Star (A\*) uses a grid of points to represent the field and uses graph exploration to connect the start and destination. As it explores each point, it checks against each of the given obstacles to see if that point is inside. A Star (A\*) determines the path length by adding up the number edges along the path from start to destination. At each iteration of graph exploration, A Star (A\*) will (1) update a grid coordinate to the predecessor grid coordinate back to the start of the path, and (2) update the path length to the path length of the predecessor back to the start + one more edge length. By tracing back from the destination via the predecessor of each grid coordinate, A Star (A\*) will connect the start and destination with the shortest path. A Star (A\*) paths are limited by how the graph is set up, namely a grid. Specifically, A Star (A\*) paths change direction at right angles since the graph is a grid. One approach to fixing right-angled paths is smoothing in post-processing, but this smoothed path no longer has guarantees about it being the shortest possible path, since A Star (A\*) optimized for the shortest right-angled path, not the shortest smoothed path.

Theta Star (Theta \*) uses the same grid and it explores the grid in the same way as A Star (A\*). The difference is when it updates the predecessor and path length at each iteration of the graph exploration. At each iteration, let's define 3 coordinates: the `current` coordinate, the `predecessor` of the current coordinate, and the `next` unexplored coordinate from the currrent coordinate. Theta Star (Theta \*) will check for line of sight (no crossing obstacles) between `predecessor` and `next`; if there is line of sight, then predecessor of `next` will become `predecessor` (of `current`) and the path length will be the length from `next` to `predecessor` + the path length of `predecessor`. The key idea is that Theta Star (Theta\*) allows any angle change in direction along the path and Theta Star (Theta \*) takes into account the Euclidean distance of diagonal segments of the path when optimizing path length.
