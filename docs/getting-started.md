...
1. Run our AI on Thunderscope:
    ...
    - If we want to run with simulated AI vs AI:
        - `./tbots.py run thunderscope_main --enable_autoref` will start Thunderscope with our Simulator, a blue FullSystem, yellow FullSystem and an AutoRef. Each FullSystem contains the respective AI for each side. The command will start Thunderscope and set up communication between the Simulator, Gamecontroller, FullSystems and Autoref.
        ...
        - The AutoRef sends RefereeCommands to both AIs, so that the AI can respond to game events. If you want to manually send RefereeCommands using the Gamecontroller, omit the `--enable_autoref` flag.
