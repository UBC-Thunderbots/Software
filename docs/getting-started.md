...
3. If AI vs AI is not running properly (eg. the visualizer does not respond after starting a play), closed ports are likely the cause. `cd` into `Software/environment_setup` and run `./disable_ufw.sh` to disable UFW and allow traffic through the visualizer ports.
