...
The `Estop` is a physical push button that is connected to the computer via a USB cable. When Thunderscope is launched, a `threaded_estop_reader.cpp` is initialized (within `robot-communication.py`) that is responsible for communicating and reading values from the `Estop` via UART. While running, it will periodically check it the `Estop` is in the `STOP` state. If this is case it overrides the [Primitives](#primitives) sent to the robots to an `Estop primitive`. On the robot, `Thunderloop` is responsible for handling the primitive message, and ensuring that the power & motor boards receive the correct inputs for the robot to stop. When the `Estop` is in the `PLAY` state, primitives are communicated normally.
