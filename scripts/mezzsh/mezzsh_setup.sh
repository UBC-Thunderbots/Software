...
sed -i '/ForceCommand \/usr\/local\/bin\/gatekeeper.sh/d' $SSHD_CONFIG
...
TIMEOUT_LINE="export TMOUT=$TIMEOUT_SECONDS && readonly TMOUT"

if [ -d "$USER_HOME" ]; then
    # Remove old TMOUT lines if they exist and append new one
    sed -i '/TMOUT/d' "$USER_HOME/.bashrc"
    echo "$TIMEOUT_LINE" >> "$USER_HOME/.bashrc"
